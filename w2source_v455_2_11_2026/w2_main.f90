...
CONTAINS
  ...
  SUBROUTINE RUN_TAIL_PREDICTOR()
    INTEGER :: JWP, JBP, IUP_PRED
    LOGICAL :: ANY_PREDICT_COMPUTED, REUSE_PREDICTOR
    REAL(R8) :: DWSE, DQLINK, VOL_MIN_PRED, Q_AVAILABLE_MAX_PRED
    ...
    DO JWP=1,NWB
      DO JBP=BS(JWP),BE(JWP)
        ...
        IF (REUSE_PREDICTOR) THEN
          IUP_PRED = TAIL_UPSEG(JBP)
          VOL_MIN_PRED = TAIL_MIN_PROFILE_VOLUME(JBP, JWP, TAIL_WSE_DN(JBP))
          Q_AVAILABLE_MAX_PRED = MAX(QIN(JBP), 0.0D0) + &
            MAX(TAIL_STORAGE_VOL(JBP)-VOL_MIN_PRED, 0.0D0)/MAX(ABS(DLT), 1.0D-12)
          IF (TAIL_Q_LINK(JBP) > Q_AVAILABLE_MAX_PRED+1.0D-9) REUSE_PREDICTOR = .FALSE.
        END IF
