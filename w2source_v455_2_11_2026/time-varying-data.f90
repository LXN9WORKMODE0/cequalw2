...
SUBROUTINE TIME_VARYING_DATA
  ...
  USE KINETIC, ONLY:EXH2O; USE SHADEC; USE MAIN, ONLY: PUMPS, SEGNUM, WBSEG, N2BND, DOBND, DGPBND,NDO, NDGP, ITR, EA, SYSTDG, NN2, TMEND, ATM_DEPOSITION, ATMDEPFN, ATM_DEP_LOADING, ATMDCN, NACATD, ATM_DEPOSITION_INTERPOLATION, GASGTC, GASSPC,CC_LW,CC_SW,I_LWAT,I_SW,SW_ALBEDO,SW_BA,SW_K1,AOD380NM,AOD500NM,TZONE ! systdg ADD WBSEG, N2BND, DOBND, NDO, NGN2, ITR, EA, SYSTDG
  USE TDGAS
...
! Type declaration
  ...
  REAL,    ALLOCATABLE, DIMENSION(:)     :: WSCNX, BPNX,EPU2,EONPU2,EOFFPU2,QPU2,NXPUMP,NXATMDEP,AGASSPNX, AGASGTNX, BGASSPNX, CGASSPNX, DGASSPNX, BGASGTNX, CGASGTNX, DGASGTNX  
...
! Allocation declarations
  ...
  IF(NGT>0)ALLOCATE(NXGTDO(NGT), FGASGT(NGT),FILE_GAS_GT(NGT),AGASGTNX(NGT),BGASGTNX(NGT), CGASGTNX(NGT), DGASGTNX(NGT))
  IF(NSP>0)ALLOCATE(NXSPDO(NSP), FGASSP(NSP),FILE_GAS_SP(NSP),AGASSPNX(NSP),BGASSPNX(NSP), CGASSPNX(NSP), DGASSPNX(NSP))
...
! Open input files
  ...
  ! Check for presence of w2_met.csv
  INQUIRE(FILE="W2_Met.csv", EXIST=INPUT_FILE_EXISTS)   ! file_exists will be TRUE if the file
  if(INPUT_FILE_EXISTS)then
      open(nunit,file='w2_Met.csv',status='old')
      read(nunit,*);read(nunit,*)
      read(nunit,*)A_JUNK,I_LWAT
      read(nunit,*)A_JUNK,CC_LW
      read(nunit,*)A_JUNK,CC_SW
      read(nunit,*)A_JUNK,I_SW
      read(nunit,*)A_JUNK,SW_ALBEDO
      read(nunit,*)A_JUNK,SW_BA
      read(nunit,*)A_JUNK,SW_K1
      read(nunit,*)A_JUNK,AOD380nm
      read(nunit,*)A_JUNK,AOD500nm
      read(nunit,*)A_JUNK,TZONE
      CLOSE(NUNIT)
  else
      cc_lw=0.0017
      cc_sw=0.0065
  ...
  ENDIF
     ...
     DO N=1,NSP              ! SW 1/18/2022
        IF(GASSPC(N)=='      ON' .AND.  EQSP(N)==4 .AND. BGASSP(N)==1)THEN
         ...
         AGASSP(N)=AGASSPNX(N)
        ELSEIF(GASSPC(N)=='      ON' .AND.  EQSP(N)==5)THEN  
         FGASSP(N)=NPT; NPT=NPT+1
         WRITE (SEGNUM,'(I0)') N 
         SEGNUM = ADJUSTL(SEGNUM)  
         L = LEN_TRIM(SEGNUM) 
         ...
         OPEN (FGASSP(N), FILE=FILE_GAS_SP(N),STATUS='OLD')
         READ(FGASSP(N),*);READ(FGASSP(N),*);READ(FGASSP(N),*)   ! SKIP 3 LINES
         READ(FGASSP(N),*)NXSPDO(N),AGASSPNX(N),BGASSPNX(N), CGASSPNX(N), DGASSPNX(N) 
         AGASSP(N)=AGASSPNX(N)
         BGASSP(N)=BGASSPNX(N)
         CGASSP(N)=CGASSPNX(N)
         DGASSP(N)=DGASSPNX(N)
    ...
    DO N=1,NGT
        IF(GASGTC(N)=='      ON' .AND. EQGT(N)==4 .AND. BGASGT(N)==1.0)THEN
         ...
         AGASGT(N)=AGASGTNX(N)
        ELSEIF(GASGTC(N)=='      ON' .AND. EQGT(N)==5)THEN
         ...
         OPEN (FGASGT(N), FILE=FILE_GAS_GT(N),STATUS='OLD')
         READ(FGASGT(N),*);READ(FGASGT(N),*);READ(FGASGT(N),*)   ! SKIP 3 LINES
         READ(FGASGT(N),*)NXGTDO(N),AGASGTNX(N),BGASGTNX(N), CGASGTNX(N), DGASGTNX(N) 
         AGASGT(N)=AGASGTNX(N)
         BGASGT(N)=BGASGTNX(N)
         CGASGT(N)=CGASGTNX(N)
         DGASGT(N)=DGASGTNX(N)
...
ENTRY DEALLOCATE_TIME_VARYING_DATA
  ...
  IF(NGT>0)DEALLOCATE(NXGTDO, FGASGT,FILE_GAS_GT,AGASGTNX,BGASGTNX, CGASGTNX, DGASGTNX)
  IF(NSP>0)DEALLOCATE(NXSPDO, FGASSP,FILE_GAS_SP,AGASSPNX,BGASSPNX, CGASSPNX, DGASSPNX)
RETURN
