...
USE GLOBAL;     USE NAMESC; USE GEOMC;  USE LOGICC; USE PREC;  USE SURFHE;  USE KINETIC; USE SHADEC; USE EDDY
  ...
  ! MACROPHYTEC, POROSITYC, ZOOPLANKTONC removed - water quality related (MACROPHYTE_CALC references are commented out)  
  IMPLICIT NONE
...
!***********************************************************************************************************************************
 ...
 ! CODE MOVED to after wse computation       ELWS(CUS(JB):DS(JB)+1) = EL(KT,CUS(JB):DS(JB)+1)-Z(CUS(JB):DS(JB)+1)*COSA(JB)
        ...
        DO I=CUS(JB)-1,DS(JB)+1
          DO K=KT,KB(I)     !DO CONCURRENT(K=KT:KB(I))             !DO K=KTWB(JW),KB(I)
            QSS(K,I)   = 0.0D0
            TSS(K,I)   = 0.0D0
            SU(K,I)    = U(K,I)
            SW(K,I)    = W(K,I)
  ...
  DO JW = 1,NWB 
    ...
    IF (CONSTITUENTS .AND. ((CUF > 0 .AND. MOD(NIT,INT(CUF)) == 0) .OR. (CUF <= 0 .AND. JDAY >= NXTMUK))) THEN      !SR 07/03/2023
