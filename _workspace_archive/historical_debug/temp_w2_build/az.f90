...
!***********************************************************************************************************************************
MODULE AZ_LOCAL
  USE GEOMC; USE GLOBAL; USE TRANS; USE PREC; USE EDDY; USE KINETIC; USE LOGICC; USE MAIN, ONLY:WARNING_OPEN, IUT,IDT
...
SUBROUTINE CALCULATE_AZ0
  ...
  IF (AZC(JW) == '    NICK') THEN
    SLM = ((DEPTHR+DEPTHL)*0.5*(0.14-0.08*(1.0-ZD)**2-0.06*(1.0-ZD)**4))**2         ! SR 7/2024
...
! macrophyte_on removed - MACROPHYTEC module deleted
  ...
  TKE(KT,I,1) = (3.33*(USTAR*USTAR+USTARBKT*USTARBKT))*BHRATIO(KT,I)   !(BH2(KT,I)/BH1(KT,I))                            ! SG 10/4/07
  TKE(KT,I,2) = (USTAR*USTAR*USTAR+USTARBKT*USTARBKT*USTARBKT)*5.0/H1(KT,I)*BHRATIO(KT,I)   !*(BH2(KT,I)/BH1(KT,I))       ! SG 10/4/07
  DO K=KT+1,KB(I)-1
...
!RODI NOWIND
	...
	DO J=1,2
	  ...
	  IF(J==1)THEN		
		...
		DT(K,I) = 0.0
	  ELSE
		TKE(KT,I,2) = TKE(K,I,1)**(1.5)/(ARODI(JW)*DEPTH)*BHRATIO(K,I)   !*BH2(KT,I)/BH1(KT,I)                                     !3.0/2.0
		AT(K,I) = 0.0
		CT(K,I) = 0.0
		VT(K,I) = 1.0
		DT(K,I) = TKE(KT,I,2)
	...
	DO JJ=1,3
	  ...
	  ELSEIF(JJ==2)THEN
		IF(TKE(KT,I,1)*0.3<(USTAR*USTAR))THEN
		  TKE(KT,I,1) = (USTAR*USTAR)*3.33333*BHRATIO(KT,I)   !*BH2(KT,I)/BH1(KT,I)
		  AT(KT,I) = 0.0
		  CT(KT,I) = 0.0
		  VT(KT,I) = 1.0
		  DT(KT,I) = TKE(KT,I,J)
	  ...
	  IF(JJ==3)THEN
		TKE(KT,I,2) = (TKE(KT,I,1)*0.3)**(1.5)/(0.41*(H2(KT,I)*0.5 + ARODI(JW)*DEPTH*   &
		              (1-(USTAR*USTAR)/(TKE(KT,I,1)*0.3))))*BHRATIO(KT,I)   !*BH2(KT,I)/BH1(KT,I)		   !3.0/2.0    /2.0
		AT(KT,I) = 0.0
		CT(KT,I) = 0.0
		VT(KT,I) = 1.0
		DT(KT,I) = TKE(KT,I,2)
...
! TOP LAYER BC
  ...
  TKE(KT,I,1) = (3.33*(USTAR*USTAR+USTARBKT*USTARBKT))*BHRATIO(KT,I)   !*(BH2(KT,I)/BH1(KT,I))
  TKE(KT,I,2) = (USTAR*USTAR*USTAR+USTARBKT*USTARBKT*USTARBKT)*5.0/H1(KT,I)*BHRATIO(KT,I)   !*(BH2(KT,I)/BH1(KT,I))
	DO J=1,2
	  K=KT			
	  AT(K,I) = 0.0
	  CT(K,I) = 0.0
