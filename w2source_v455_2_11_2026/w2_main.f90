    ...
    DO JW=1,NWB
      DO JB=BS(JW),BE(JW)
        ...
        IF (TAIL_REACH_INITIALIZED_SAVE(JB)) THEN
          ...
          TAIL_FULL_STORAGE_RESID = TAIL_FULL_STORAGE_RATE-(TAIL_Q_INFLOW(JB)-TAIL_Q_LINK(JB))
          TAIL_STORAGE_RATE_GAP = TAIL_FULL_STORAGE_RATE-TAIL_STORAGE_CHANGE_RATE(JB)
          WARNING_OPEN = .TRUE.
          WRITE (WRN,'(A,1X,A,I0,5(1X,A,ES12.4))') '[V31_SINGLE_STEP]', 'JB=',JB, &
...
CONTAINS
  ...
  SUBROUTINE UPDATE_TAIL_STAGE(JB_IN, JW_IN, ETA_INTERFACE_GUESS, Q_INTERFACE_COMMIT)
    ...
    TAIL_MASS_RESIDUAL(JB_IN) = TAIL_STORAGE_CHANGE_RATE(JB_IN)-(QPHYS-TAIL_Q_LINK(JB_IN))
