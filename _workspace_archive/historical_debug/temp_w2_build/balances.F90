...
USE GLOBAL;     USE NAMESC; USE GEOMC;  USE LOGICC; USE PREC;  USE SURFHE;  USE KINETIC; USE SHADEC; USE EDDY
  ...
  ! USE KINETIC kept - SEDP, SEDN, TP, TN, etc. used but guarded by CONSTITUENTS/SEDIMENT_DIAGENESIS checks
  ! USE MACROPHYTEC kept - MAC, MACROPHYTE_CALC used but guarded by MACROPHYTE_ON check
  ! USE TDGAS removed - not used in balances
  ! USE POROSITYC, ZOOPLANKTONC removed - water quality related
  ! Use CEMAVars removed - sediment diagenesis
  IMPLICIT NONE
...
!***********************************************************************************************************************************
    ...
    !QOUTT = 0.0
    VOLSR = 0.0
    VOLTR = 0.0; Surface_Area=0.0; Volume_WB=0.0    ! SW 1/11/2026
    ...
    TPPLANT=0.0
      ...
      KT = KTWB(JW)
        ...
        !NXFLOWBAL = NXFLOWBAL+FLOWBALF  
            ...
            ! Compute volume and surface area of each waterbody   ! SW 1/11/2026
            
            DO JB=BS(JW),BE(JW)
            IF(BR_INACTIVE(JB))CYCLE
                 DO I=CUS(JB),DS(JB)
                    Surface_Area(JW)=Surface_Area(JW)+DLX(I)*B(KTI(I),I)
                        DO K=KT,KB(I)
                        Volume_WB(JW)=Volume_WB(JW)+BH1(K,I)*DLX(I)
                        END DO ! K
                 ENDDO   ! I
            END DO ! JB
            
            IF(VOLUME_BALANCE(JW))THEN
            WRITE(FLOWBFN,'(F10.3,",",1X,I3,",",11(E16.8,",",1X))')JDAY,JW,VOLINJW,VOLPRJW,VOLOUTJW,VOLWDJW,VOLEVJW,VOLDTJW,VOLTRBJW,VOLICEJW,Surface_Area(JW),Volume_WB(JW),DLVR(JW)
            ELSE
            WRITE(FLOWBFN,'(F10.3,",",1X,I3,",",10(E16.8,",",1X))')JDAY,JW,VOLINJW,VOLPRJW,VOLOUTJW,VOLWDJW,VOLEVJW,VOLDTJW,VOLTRBJW,VOLICEJW,Surface_Area(JW),Volume_WB(JW)
            ENDIF
        ENDIF
