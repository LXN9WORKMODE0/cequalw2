...
## Step 25 — V42 2020—2025 multiyear component validation

Status: complete; original-window mechanism retained, universal fixed-NEFF hypothesis rejected

Input and QA:

- 新增水位、流量工作簿均有 2020-01-01 至 2026-01-01 的完整逐小时时间轴；原始文件只读并记录 SHA-256。
- 水位执行 `500—650 m`、孤立 `5 m` 跳点和宽松空间异常检查；BHT 隔小时流量只补单个内部小时，并保留原测/插值标签。
- segment 28 边界用 SJ segment 26 与 YMT segment 57 按模型距离插值，不使用待预测 BHT 水位。
- 2021—2025 完整输入覆盖分别为 `99.87%/97.67%/97.10%/99.86%/99.32%`；2020 只有 `2.45%`，不能解释为完整年份。

Implementation verification:

- Python 逐项复刻 `TAIL_SECTION_PROPS`、线性 profile、断面积分阻力、速度水头、生产版 stage root expansion、`+25 m` 上限和 profile storage。
- 复刻器在首个 V38 日志点得到 `5363.09 m3/s`，日志为 `5360.00 m3/s`；差异与日志水位的 `0.01 m` 舍入一致。
- 8 个单元测试覆盖几何、日志点、完整阻力反解、stage round trip、storage 单调性和跳点质控，全部通过。

Evidence:

- 原 V38 selection、V40 strict holdout、V39 full-window 的准稳态 BHT RMSE 为 `0.747/0.637/0.652 m`，支持保留原窗口机制。
- 冻结 `NEFF=0.0725` 后，2021—2025 全年 BHT RMSE 为 `1.814/1.820/1.528/1.279/1.013 m`。
- 低流量+低水位所需逐时等效系数中位数为 `0.029—0.048`；高流量+低水位为 `0.065—0.073`，固定系数不具备跨工况自适应性。
- 正水头但超过 `25 m` 的小时在 2021/2022/2023 年分别为 `1341/762/1826`；水头不大于 `0.05 m` 的小时分别为 `800/743/57`，这些状态暴露当前闭合的结构边界。
- 用 2/6/12/24 小时 profile-storage 差分修正连续方程后，2021—2025 流量偏差仍约 `-594—-705 m3/s`，工况偏差不只是蓄水变化造成。
- 直接用 SJ 边界或只用原测流量均不改变结论。

Full-model forcing audit:

- 活动案例原 JDAY `44430—44484` 窗口的 12 个流量、12 个温度、13 个气象和 1 个水库出流文件全部覆盖。
- 2020—2025 没有任何完整年份同时覆盖全部活动强迫；因此未伪造气象、温度、支流或调度输入来运行六个完整 W2 年份。

Decision:

- 保留默认关闭的 BHT—SJ 守恒单控制体；`0.0725` 降级为 V38—V41 可复现参考值，不再视为跨年推荐参数。
- 不依据验证集把全局系数改成 `0.070` 或逐年中位数；这会把已证实的状态依赖重新压成补丁常数。
- 方案 C 已获得继续设计的证据，但下一步先核对 BHT/SJ/YMT 高程基准并补齐完整跨年强迫；若状态依赖仍存在，再进入保守多状态/一维非恒定流子模型。

Reference:

- `docs/reference/hydrodynamics_limitations/58_v42_multiyear_cross_validation.md`
