...
当前方案已经从“局部调参实验”发展成了一套结构上正确、物理上闭合、数值上稳定、能够被关闭和复现、并且得到跨年组件证据支持的模型扩展。
...
V42 跨年组件检验后，当前最准确的定位是：

> 这是一个在原 2021 窗口内有效、结构守恒的工程折中方案；跨年数据不支持把固定 `NEFF=0.0725` 当成跨工况最终生产参数。

因此，后续最正确的动作不是继续追求把 `0.0725` 调到小数点后更多位，也不是利用验证集另选一个固定常数。数据提供者已经确认水位采用同一高程基准、BHT 流量与水位可按北京时间整点同步、BHT 对应 segment 2，且 segment 2—27 内无模型化支流接口；历史 distributed 流量只是虚拟水位校正量。既然这些数据语义已澄清而工况依赖仍存在，下一步应把当前单状态控制体升级为显式包含惯性、内部水面和分段蓄水的保守多状态/一维子模型。只有要验证完整 CE-QUAL-W2 时，才必须另行补齐跨年气象、水温和物理支流强迫。完整结果见 [V42 跨年组件检验](../reference/hydrodynamics_limitations/58_v42_multiyear_cross_validation.md)。
