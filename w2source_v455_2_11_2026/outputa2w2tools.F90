SUBROUTINE OUTPUTA  
  ...
  ! MACROPHYTEC removed; USE POROSITYC; USE ZOOPLANKTONC;   USE BIOENERGETICS
  ...
  DO JW=1,NWB  
      ...
      IF (ANY(DIST_TRIBS(BS(JW):BE(JW)))) THEN                                                                      !SR 11/30/2021
        ...
        IF (MASS_BALANCE(JW)) THEN  
          ...
          DO JB=BS(JW),BE(JW)  
            ...
            ! MACROPHYTEC DO M=1,NMC block removed
                 ...
                 DO JJ=1,NWB
                     ...
                     DO JB=BS(JJ),BE(JJ)
                         DO I=CUS(JB),DS(JB)
                             ...
                             ELSE
                            WRITE(NUNIT,'(I5,",",*(F12.4,","))')I,ELWS(I),QC(I),T2(K,I),DEPTHB(KB(I),I),B(KTI(I),I),TVOLAVG
                            ENDIF
                         ENDDO
                     ENDDO
                 ENDDO
    ...
    IF (SPREADSHEET(JW)) THEN  
      IF (JDAY >= NXTMSP(JW) .OR. JDAY >= SPRD(SPRDP(JW)+1,JW)) THEN  
        ...
        WRITE (SPRV(JW),'(A,",",F12.4,",",*(A,","))') '"Temperature(C)                             "',JDAY,(CONV1(K,ISPR(J,JW)),J=1,NISPR(JW))     ! SW 6/2025
            ...
            DO J=1,NISPR(JW)  
                                       TVOLAVG=0.0     ! SW 8/30/2018
                                        ...
                                        ELSE
                                            TVOLAVG=-99.0
                                        ENDIF
                WRITE (CONV1(K,ISPR(J,JW)),'(ES12.4)') TVOLAVG                                                ! f12.4 SW 8/31/18; 2/11/2026  
            END DO   
           !   WRITE (SPRV(JW),'(A,",",F10.3,",",*(A,","))') trim(CNAME3(CN(JC))),JDAY,(CONV1(K,ISPR(J,JW)),J=1,NISPR(JW))   
            WRITE (SPRV(JW),'(A43,",",F12.4,",",*(A,","))') CNAME(CN(JC)),JDAY,(CONV1(K,ISPR(J,JW)),J=1,NISPR(JW))   ! SW 6/2025
          END IF  
        END DO  
        IF (CONSTITUENTS) THEN  
          DO JD=1,NACD(JW)  
            IF (PRINT_DERIVED(CDN(JD,JW),JW)) THEN  
              ...
              END DO  
                ...
                WRITE (SPRV(JW),'(A43,",",F12.4,",",*(A,","))') CDNAME(CDN(JD,JW)),JDAY,(CONV1(K,ISPR(J,JW)),J=1,NISPR(JW))  ! SW 6/2025
            END IF  
            END DO  
            END IF    
        ...
        DO K=KTWB(JW),KBMAX(JW)  
          ...
          WRITE (SPR(JW),'(A,",",2(F12.4,","),*(F10.3,",",A,","))') '"Temperature(C)                             "',JDAY,DEPTHM(K,DS(BS(JW))),     &    ! SW 6/2025
        ...
        WRITE (SPR(JW),'(A,",",2(F12.4,","),*(F10.3,",",A,","))') '"HorizontalVelocity(m/s)                    "',JDAY,DEPTHM(K,DS(BS(JW))),      &    ! SW 6/2025
          (ELWS(ISPR(J,JW))-DEPTHM(K,ISPR(J,JW)),CONV1(K,J),J=1,NISPR(JW))  
        END DO  
        DO J=1,NISPR(JW)  
          DO K=KTWB(JW),KB(ISPR(J,JW))  
            WRITE (CONV1(K,J),'(ES12.4)') U(K,ISPR(J,JW))*BHR(K,ISPR(J,JW))                    !F12.4
          END DO  
        ENDDO
        ...
        DO K=KTWB(JW),KBMAX(JW)  
          !WRITE (SPR(JW),'(A,",",2(F10.3,","),*(F10.3,",",A,","))') 'HorizontalLayerFlow(m3s-1)',JDAY,DEPTHM(K,DS(BS(JW))),                      & 
           WRITE (SPR(JW),'(A,",",2(F12.4,","),*(F10.3,",",A,","))') '"HorizontalLayerFlow(m3/s)                  "',JDAY,DEPTHM(K,DS(BS(JW))),     &    ! SW 6/2025           
          (ELWS(ISPR(J,JW))-DEPTHM(K,ISPR(J,JW)),CONV1(K,J),J=1,NISPR(JW))  
        END DO  
        ...
        DO JC=1,NAC  
          IF (PRINT_CONST(CN(JC),JW)) THEN  
            ...
            DO K=KTWB(JW),KBMAX(JW)  
              ...
              WRITE (SPR(JW),'(A45,",",2(F12.4,","),*(F10.3,",",A,","))') '"'//CNAME(CN(JC))//'"',JDAY,DEPTHM(K,DS(BS(JW))),                            &  ! SW 6/2025
                  (ELWS(ISPR(J,JW))-DEPTHM(K,ISPR(J,JW)),CONV1(K,J),J=1,NISPR(JW))  
            END DO  
          END IF  
        END DO  
          ...
          DO JD=1,NACD(JW)  
            IF (PRINT_DERIVED(CDN(JD,JW),JW)) THEN  
              ...
              DO K=KTWB(JW),KBMAX(JW)  
                ...
                WRITE (SPR(JW),'(A45,",",2(F12.4,","),*(F10.3,",",A,","))') '"'//CDNAME(CDN(JD,JW))//'"',JDAY,DEPTHM(K,DS(BS(JW))),                     &  
                    (ELWS(ISPR(J,JW))-DEPTHM(K,ISPR(J,JW)),CONV1(K,J),J=1,NISPR(JW))  
              END DO  
            END IF  
          END DO  
    ...
    IF (FLUX(JW)) THEN  
      IF (JDAY >= NXTMFL(JW) .OR. JDAY >= FLXD(FLXDP(JW)+1,JW)) THEN  
        ...
        ENDIF
          ...
          END DO  
        END DO  
        WRITE(FLX2(JW),'(F12.4,",",f8.3,",",*(E12.4,","))')JDAY,ELTMF(JW)/DAY,(KFJW(JW,KFCN(K,JW)),K=1,NAF(JW))  
        ELTMF(JW)                    = 0.0  
          ...
          DO JB=BS(JW),BE(JW)  
            IF (DS(JB) == IWDO(J)) THEN  
              ...
              DO JS=1,NSTR(JB)  
                ...
                WRITE(WDO2(JFILE,1),'(F12.4,",",F12.4)')JDAY,QSTR(JS,JB)  
                WRITE(WDO2(JFILE,2),'(F12.4,",",F8.3)')JDAY,TAVG(JS,JB)  
                IF (CONSTITUENTS)then
                  cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qstr(js,jb)*cavg(js,jb,cn(1:nac))                 ! cb 1/16/13
                  WRITE (WDO2(JFILE,3),'(F12.4,",",*(ES13.5,","))') JDAY,(CAVG(JS,JB,CN(JC)),     JC=1,NAC)  
                end if
                IF (DERIVED_CALC)then
                  cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qstr(js,jb)*cdavg(js,jb,cdn(1:nacd(jw),jw))                 ! cb 1/16/13 
                  WRITE (WDO2(JFILE,4),'(F12.4,",",*(ES13.5,","))') JDAY,(CDAVG(JS,JB,CDN(JD,JW)),JD=1,NACD(JW))  
                end if
              ENDDO  
              JSSS(JB)=NSTR(JB)  
            END IF  
   ...
   !     JWD=0                        ! sw 5/17/13
        ...
        DO JJ=1,NWD  
          ...
          IF (IWD(JWD) == IWDO(J)) THEN  
            ...
            WRITE(WDO2(JFILE,1),'(F12.4,",",F12.4)')JDAY,QWD(JWD)  
            WRITE(WDO2(JFILE,2),'(F12.4,",",F8.2)')JDAY,TAVGW(JWD)  
            IF (CONSTITUENTS)then
              cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qwd(jwd)*cavgw(jwd,cn(1:nac))                 ! cb 1/16/13  
              WRITE (WDO2(JFILE,3),'(F12.4,",",*(ES13.5,","))') JDAY,(CAVGW(JWD,CN(JC)),     JC=1,NAC)  
            end if
            IF (DERIVED_CALC)THEN  
              !DO JW=1,NWB  
              !  IF (IWDO(J) >= US(BS(JW)) .AND. IWDO(J) <= DS(BE(JW))) EXIT  
              ...
              WRITE (WDO2(JFILE,4),'(F12.4,",",*(ES13.5,","))') JDAY,(CDAVGW(JWD,CDN(JD,JW)),JD=1,NACD(JW))  
            ENDIF  
          ENDIF  
        ENDDO  
        DO JS=1,NSP  ! spillways
          ...
          IF(IWDO(J) == IUSP(JS))THEN  
            JFILE=JFILE+1  
            WRITE(WDO2(JFILE,1),'(F12.4,",",F12.4)')JDAY,QSP(JS)  
            IF(LATERAL_SPILLWAY(JS))THEN  
            !  JWD=JWD+1
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F12.4,",",*(ES13.5,","))') JDAY,(CDAVGW(JWD,CDN(JD,JWUSP(JS))),JD=1,NACD(JWUSP(JS)))  
              end if
            ELSE  
            !  JSSS(JBUSP(JS))=JSSS(JBUSP(JS))+1
                qwdo(j)           = qwdo(j)            +qsp(js)                 ! cb 1/16/13  
                ...
                ENDIF
              WRITE(WDO2(JFILE,2),'(F12.4,",",F8.2)')JDAY,TAVG(JSSS(JBUSP(JS)),JBUSP(JS))  
              IF (CONSTITUENTS)then
                cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qsp(js)*cavg(jsss(jbusp(js)),jbusp(js),cn(1:nac))            ! cb 1/16/13
                WRITE (WDO2(JFILE,3),'(F12.4,",",*(ES13.5,","))') JDAY,(CAVG(JSSS(JBUSP(JS)),JBUSP(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F12.4,",",*(ES13.5,","))') JDAY,(CDAVG(JSSS(JBUSP(JS)),JBUSP(JS),CDN(JD,JWUSP(JS))),JD=1,NACD(JWUSP(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
        DO JS=1,NPU  ! PUMP
          ...
          IF(IWDO(J) == IUPU(JS))THEN  
            JFILE=JFILE+1  
            IF(PUMPON(JS))THEN  
              WRITE(WDO2(JFILE,1),'(F12.4,",",F12.4)')JDAY,QPU(JS)  
            ELSE  
              WRITE(WDO2(JFILE,1),'(F12.4,",",F8.3)')JDAY,0.0  
            ENDIF  
            IF(LATERAL_PUMP(JS))THEN  
            !  JWD=JWD+1              
                IF(QPU(JS)==0.0)THEN
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F12.4,",",*(ES13.5,","))') JDAY,(CDAVGW(JWD,CDN(JD,JWUPU(JS))),JD=1,NACD(JWUPU(JS)))  
              end if
            ELSE  
            !  JSSS(JBUPU(JS))=JSSS(JBUPU(JS))+1  
                IF(QPU(JS)==0.0)THEN
              ...
              WRITE(WDO2(JFILE,2),'(F12.4,",",F8.2)')JDAY,TAVG(JSSS(JBUPU(JS)),JBUPU(JS))  
              IF (CONSTITUENTS)then
                if(pumpon(js))then                ! cb 1/16/13
                  cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qpu(js)*cavg(jsss(jbupu(js)),jbupu(js),cn(1:nac)) 
                end if
                WRITE (WDO2(JFILE,3),'(F12.4,",",*(ES13.5,","))') JDAY,(CAVG(JSSS(JBUPU(JS)),JBUPU(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                if(pumpon(js))then   ! cb 1/16/13
                  cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qpu(js)*cdavg(jsss(jbupu(js)),jbupu(js),cdn(1:nacd(jwupu(js)),jwupu(js))) 
                end if
                WRITE (WDO2(JFILE,4),'(F12.4,",",*(ES13.5,","))') JDAY,(CDAVG(JSSS(JBUPU(JS)),JBUPU(JS),CDN(JD,JWUPU(JS))),JD=1,NACD(JWUPU(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
        DO JS=1,NPI  ! pipes
          ...
          IF(IWDO(J) == IUPI(JS))THEN  
            JFILE=JFILE+1  
            WRITE(WDO2(JFILE,1),'(F12.4,",",F12.4)')JDAY,QPI(JS)  
            IF(LATERAL_PIPE(JS))THEN  
           !   JWD=JWD+1  
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F12.4,*(ES13.5,","))') JDAY,(CDAVGW(JWD,CDN(JD,JWUPI(JS))),JD=1,NACD(JWUPI(JS)))  
              end if
            ELSE  
            !  JSSS(JBUPI(JS))=JSSS(JBUPI(JS))+1
                qwdo(j)           = qwdo(j)            +qpi(js)     ! cb 1/16/13
                ...
                ENDIF
              WRITE(WDO2(JFILE,2),'(F12.4,",",F8.2)')JDAY,TAVG(JSSS(JBUPI(JS)),JBUPI(JS))  
              IF (CONSTITUENTS)then
                cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qpi(js)*cavg(jsss(jbupi(js)),jbupi(js),cn(1:nac))     ! cb 1/16/13
                WRITE (WDO2(JFILE,3),'(F12.4,",",*(ES13.5,","))') JDAY,(CAVG(JSSS(JBUPI(JS)),JBUPI(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F12.4,",",*(ES13.5,","))') JDAY,(CDAVG(JSSS(JBUPI(JS)),JBUPI(JS),CDN(JD,JWUPI(JS))),JD=1,NACD(JWUPI(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
        DO JS=1,NGT  ! gates
          ...
          IF(IWDO(J) == IUGT(JS))THEN  
            JFILE=JFILE+1  
            WRITE(WDO2(JFILE,1),'(F12.4,",",F12.4)')JDAY,QGT(JS)  
            IF(LATERAL_GATE(JS))THEN  
          !    JWD=JWD+1  
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F12.4,",",*(ES13.5,","))') JDAY,(CDAVGW(JWD,CDN(JD,JWUGT(JS))),JD=1,NACD(JWUGT(JS)))  
              end if
            ELSE  
           !   JSSS(JBUGT(JS))=JSSS(JBUGT(JS))+1
               qwdo(j)           = qwdo(j)            +qgt(js)     ! cb 1/16/13
               twdo(j)           = twdo(j)            +qgt(js)*tavg(jsss(jbugt(js)),jbugt(js))
                ...
                ENDIF
              WRITE(WDO2(JFILE,2),'(F12.4,",",F8.2)')JDAY,TAVG(JSSS(JBUGT(JS)),JBUGT(JS))  
              IF (CONSTITUENTS)then
                cwdo(cn(1:nac),j)= cwdo(cn(1:nac),j)+qgt(js)*cavg(jsss(jbugt(js)),jbugt(js),cn(1:nac))     ! cb 1/16/13
                WRITE (WDO2(JFILE,3),'(F12.4,",",*(ES13.5,","))') JDAY,(CAVG(JSSS(JBUGT(JS)),JBUGT(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F12.4,",",*(ES13.5,","))') JDAY,(CDAVG(JSSS(JBUGT(JS)),JBUGT(JS),CDN(JD,JWUGT(JS))),JD=1,NACD(JWUGT(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
...
!          WRITE (CDWDOC(CDN(JD,JW)),'(F10.4)') CDWDO(CDN(JD,JW),J)              ! SW 9/23/13 Changed format from G8.3 to F8.3 to avoid format overflow
        ...
        END DO  
2499      WRITE (WDO(J,1),'(F12.4,",",F12.4,",",8X,*(F10.3,","))',ERR=2500) JDAY, QWDO(J), (QOUTLET(I),I=1,NUMOUTLETS)     ! sw 3/2019 This code was necessary during multiple WB read/write possible error with one file copying and another writing at the same time
          GO TO 2501
2500      CALL SLEEPQQ(100)          
          ...
          go to 2499
2501      CONTINUE
          WRITE (WDO(J,2),'(F12.4,",",F8.2,",",8X,*(F8.2,","))',ERR=2502) JDAY, TWDO(J), (TOUTLET(I),I=1,NUMOUTLETS)  
          GO TO 2503
2502      CALL SLEEPQQ(100)          
          ...
          go to 2501
2503      CONTINUE
          IF (CONSTITUENTS) WRITE (WDO(J,3),'(F12.4,",",*(A15,","))',ERR=2504) JDAY,(CWDOC(CN(JC)),     JC=1,NAC)  
          GO TO 2505
2504      CALL SLEEPQQ(100)          
          ...
          go to 2503
2505      CONTINUE
          IF (DERIVED_CALC) WRITE (WDO(J,4),'(F12.4,",",*(A15,","))') JDAY,(CDWDOC(CDN(JD,JW)),JD=1,NACD(JW))  
