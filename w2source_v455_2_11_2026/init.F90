...
USE GLOBAL;     USE NAMESC; USE GEOMC;  USE LOGICC; USE PREC;  USE SURFHE;  USE KINETIC; USE SHADEC; USE EDDY
  ...
  ! CEMAVARS, ALGAE_TOXINS removed - sediment/algae related
  ! CEMASedimentDiagenesis removed - sediment diagenesis
  IMPLICIT NONE
...
!***********************************************************************************************************************************
  ...
  RS=0.0;RN=0.0;RB=0.0;RE=0.0;RC=0.0;RANLW=0.0;TICAP=0.0
#ifdef ZOOPLANKTONC
  TICZR=0.0
#endif
  TICEP=0.0
  ...
  IUPHYS  = 0; CUSLAST = -999; CUSMIN = 0
  FRONT_SEG = 0; FRONT_STATE = FRONT_STATE_DRY
  FRONT_WET_COUNT = 0; FRONT_DRY_COUNT = 0
  TAIL_UPSEG = 0; TAIL_DNSEG = 0; TAIL_COUPLE_SEG = 0; TAIL_STAGE_MODE = 0
  TAIL_DOMAIN_US = 0; TAIL_DOMAIN_DS = 0; TAIL_DOMAIN_NSEG = 0
  FRONT_WSE = 0.0D0; FRONT_DEPTH = 0.0D0
  FRONT_AREA = 0.0D0; FRONT_VOLUME = 0.0D0; FRONT_HRAD = 0.0D0
  FRONT_TRANSITION = 0.0D0; FRONT_DRAG_FACTOR = 1.0D0; FRONT_GRAV_FACTOR = 1.0D0
  TAIL_WSE_UP = 0.0D0; TAIL_WSE_DN = 0.0D0; TAIL_Q_LINK = 0.0D0; TAIL_DEPTH_UP = 0.0D0
  TAIL_STORAGE_VOL = 0.0D0; TAIL_Q_INFLOW = 0.0D0; TAIL_Q_OUTFLOW = 0.0D0
  TAIL_Q_STATE = 0.0D0; TAIL_Q_TARGET = 0.0D0
  TAIL_TRAVEL_TIME = 0.0D0; TAIL_WAVE_CELERITY = 0.0D0; TAIL_REACH_LENGTH = 0.0D0
  TAIL_TRANSITION_STATE = 0.0D0; TAIL_SUBMERGENCE = 0.0D0; TAIL_LOCAL_SLOPE = 0.0D0; TAIL_FROUDE = 0.0D0
  TAIL_IFACE_ETA_PRED = 0.0D0; TAIL_IFACE_ETA_CORR = 0.0D0; TAIL_IFACE_Q_PRED = 0.0D0; TAIL_IFACE_Q_CORR = 0.0D0
  TAIL_IFACE_DETA = 0.0D0; TAIL_IFACE_DQ = 0.0D0; TAIL_IFACE_MAX_DETA = 0.0D0; TAIL_IFACE_MAX_DQ = 0.0D0
  TAIL_IFACE_ITER_COUNT = 0; TAIL_IFACE_CONVERGED = .FALSE.; TAIL_IFACE_RELAX = 0.50D0
  TAIL_IFACE_ETA_GUESS = 0.0D0; TAIL_IFACE_Q_GUESS = 0.0D0; TAIL_IFACE_ETA_PREV = 0.0D0; TAIL_IFACE_Q_PREV = 0.0D0
  TAIL_IFACE_DETA_STEP = 0.0D0; TAIL_IFACE_DQ_STEP = 0.0D0
  TAIL_RESERVOIR_Q_USED = 0.0D0; TAIL_STORAGE_CHANGE_RATE = 0.0D0
  TAIL_MASS_RESIDUAL = 0.0D0; TAIL_SEGMENT_Q_LOSS = 0.0D0
  TAIL_MASS_RESIDUAL_MAX = 0.0D0; TAIL_FLUX_GAP_MAX = 0.0D0
  TAIL_COMBINED_MASS_RESIDUAL_MAX = 0.0D0; TAIL_SEGMENT_Q_LOSS_MAX = 0.0D0
  TAIL_CONTROL_MODE = 0
  TAIL_STAGE_SEG = 0.0D0; TAIL_DEPTH_SEG = 0.0D0; TAIL_AREA_SEG = 0.0D0
  TAIL_HRAD_SEG = 0.0D0; TAIL_VOL_SEG = 0.0D0
  TAIL_Q_SEG = 0.0D0; TAIL_Q_TARGET_SEG = 0.0D0
  TAIL_TRAVEL_TIME_SEG = 0.0D0; TAIL_CELERITY_SEG = 0.0D0
  TAIL_SEG_VALID = .FALSE.
  TAIL_FIXED_MIN_NSEG = 4
  TAIL_MACRO_INTERFACE_SEG = 0
  TAIL_MACRO_ACTIVE_SEG = 0
  TAIL_MACRO_ACTIVE_MANNING = 0.0D0
  UPSTREAM_DOMAIN_LOCK = .FALSE.
  TAIL_COUPLED = .FALSE.
  TAIL_MULTI_SEGMENT = .FALSE.
  TAIL_DOMAIN_DEFINED = .FALSE.
  TAIL_STAGE_VALID = .FALSE.
  TAIL_REACH_INITIALIZED = .FALSE.
  TAIL_Q_STATE_INITIALIZED = .FALSE.
  BOUNDARY_INIT_LOGGED = .FALSE.
  SOURCE_DIAG_LOGGED   = .FALSE.
  FRONT_SETUP_LOGGED   = .FALSE.
  IF (.NOT. RESTART_IN) THEN
    ...
    ! MACROPHYTEC variables removed - module deleted
#ifdef MACROPHYTEC
    KTICOL=.FALSE.
#endif
  END IF
