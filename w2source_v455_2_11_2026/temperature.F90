...
USE GLOBAL;     USE NAMESC; USE GEOMC;  USE LOGICC; USE PREC;  USE SURFHE;  USE KINETIC; USE SHADEC; USE EDDY
  USE STRUCTURES; USE TRANS;  USE TVDC;   USE SELWC;  USE GDAYC; USE SCREENC; USE RSTART
  USE MetFileRegion
  ! USE KINETIC kept - EXH2O, BETA, GAMMA used for light extinction
  ! TDGAS removed - not used in temperature
  ! MACROPHYTEC, POROSITYC, ZOOPLANKTONC removed - water quality related
  ! CEMAVars removed - sediment diagenesis
  IMPLICIT NONE
...
DO JW=1,NWB
      ...
      IF (.NOT. NO_HEAT(JW)) THEN
        ...
        IF (TERM_BY_TERM(JW))THEN                                      ! SW 1/25/05
                         IF(I_LWAT==1)THEN  ! Prata (1996)
                             EA = 0.133322*DEXP(2.3026D0*(7.5D0*TDEW(JW)/(TDEW(JW)+237.3D0)+0.6609D0))   ! mmHg X 0.133322 ==kPa
                             RANLW(JW) = 0.0000000562*((273.15D0+TAIR(JW))**4)*(1.0-(1.0+(465.0*EA)/(273.15+TAIR(JW)))*EXP(-SQRT(1.2+(3*465*EA)/(273.15+TAIR(JW)))))*(1.0D0+CC_LW*CLOUD(JW)*CLOUD(JW))*0.97D0   
                         ELSEIF(I_LWAT==2)THEN  ! Idso (1981)
                             EA = 0.133322*DEXP(2.3026D0*(7.5D0*TDEW(JW)/(TDEW(JW)+237.3D0)+0.6609D0))   ! mmHg X 0.133322 ==kPa
                             RANLW(JW) = 0.0000000562*((273.15+TAIR(JW))**4)*(0.7+0.000595*EA*EXP(1500/(273.15+TAIR(JW))))*(1.0D0+CC_LW*CLOUD(JW)*CLOUD(JW))*0.97D0
                         ELSEIF(I_LWAT==3)THEN  ! Dilley and O'Brien(1998)
                             EA = 0.133322*DEXP(2.3026D0*(7.5D0*TDEW(JW)/(TDEW(JW)+237.3D0)+0.6609D0))   ! mmHg X 0.133322 ==kPa
                             RANLW(JW) = 59.83+113.7*(((273.16+TAIR(JW))/(273.16))**6)+96.96*SQRT(4650*EA/(273.15+TAIR(JW))/25)*(1.0D0+CC_LW*CLOUD(JW)*CLOUD(JW))*0.97D0
                         ELSE     ! IDSO AND JACKSON AND SWINBANK AT LOW TEMP BELOW 5C  Legacy W2 version
                           IF(TAIR(JW).GE.5.0)THEN
                           RANLW(JW) = 5.31D-13*(273.15D0+TAIR(JW))**6*(1.0D0+CC_LW*CLOUD(JW)*CLOUD(JW))*0.97D0    ! SW 4/20/16 SPEED
                           ELSE
                           RANLW(JW) = 5.62D-8*(273.15D0+TAIR(JW))**4*(1.D0-0.261D0*DEXP(-7.77D-4*TAIR(JW)*TAIR(JW)))*(1.0D0+CC_LW*CLOUD(JW)*CLOUD(JW))*0.97D0     ! SW 4/20/16 SPEED
                           ENDIF
                         ENDIF      
        ENDIF
      END IF
        ...
        IF (.NOT. NO_HEAT(JW)) THEN
          DO I=IU,ID
              if(Met_Regions)then
                        ...
                        IF (TERM_BY_TERM(JW))THEN                                      ! SW 1/25/05
                         IF(I_LWAT==1)THEN
                             EA = 0.133322*DEXP(2.3026D0*(7.5D0*TDEW(NMet)/(TDEW(NMet)+237.3D0)+0.6609D0))   ! mmHg X 0.133322 ==kPa
                             RANLW(NMet) = 0.0000000562*((273.15D0+TAIR(NMet))**4)*(1.0-(1.0+(465.0*EA)/(273.15+TAIR(NMet)))*EXP(-SQRT(1.2+(3*465*EA)/(273.15+TAIR(NMet)))))*(1.0D0+CC_LW*CLOUD(NMet)*CLOUD(NMet))*0.97D0      !CC_LW=0.0017D0 default
                         ELSEIF(I_LWAT==2)THEN  ! Idso (1981)
                             EA = 0.133322*DEXP(2.3026D0*(7.5D0*TDEW(NMet)/(TDEW(NMet)+237.3D0)+0.6609D0))   ! mmHg X 0.133322 ==kPa
                             RANLW(NMet) = 0.0000000562*((273.15+TAIR(NMet))**4)*(0.7+0.000595*EA*EXP(1500/(273.15+TAIR(NMet))))*(1.0D0+CC_LW*CLOUD(NMet)*CLOUD(NMet))*0.97D0
                         ELSEIF(I_LWAT==3)THEN  ! Dilley and O'Brien(1998)
                             EA = 0.133322*DEXP(2.3026D0*(7.5D0*TDEW(NMet)/(TDEW(NMet)+237.3D0)+0.6609D0))   ! mmHg X 0.133322 ==kPa
                             RANLW(NMet) = 59.83+113.7*(((273.16+TAIR(NMet))/(273.16))**6)+96.96*SQRT(4650*EA/(273.15+TAIR(NMet))/25)*(1.0D0+CC_LW*CLOUD(NMet)*CLOUD(NMet))*0.97D0
                         ELSE     ! IDSO AND JACKSON AND SWINBANK AT LOW TEMP BELOW 5C  Legacy W2 version
                           ...
                           ELSE
                           RANLW(NMet) = 5.62D-8*(273.15D0+TAIR(NMet))**4*(1.D0-0.261D0*DEXP(-7.77D-4*TAIR(NMet)*TAIR(NMet)))*(1.0D0+CC_LW*CLOUD(NMet)*CLOUD(NMet))*0.97D0     ! SW 4/20/16 SPEED
                           ENDIF
                         ENDIF
              ...
              !           IF(I_LWAT==1)THEN
              !               EA = 0.133322*DEXP(2.3026D0*(7.5D0*TDEW(JW)/(TDEW(JW)+237.3D0)+0.6609D0))   ! mmHg X 0.133322 ==kPa
              !               RANLW(JW) = 0.0000000562*((273.15D0+TAIR(JW))**4)*(1.0-(1.0+(465.0*EA)/(273.15+TAIR(JW)))*EXP(-SQRT(1.2+(3*465*EA)/(273.15+TAIR(JW)))))*(1.0D0+CC_LW*CLOUD(JW)*CLOUD(JW))*0.97D0      !CC_LW=0.0017D0 default
              !           ELSEIF(I_LWAT==2)THEN  ! Idso (1981)
              !               EA = 0.133322*DEXP(2.3026D0*(7.5D0*TDEW(JW)/(TDEW(JW)+237.3D0)+0.6609D0))   ! mmHg X 0.133322 ==kPa
              !               RANLW(JW) = 0.0000000562*((273.15+TAIR(JW))**4)*(0.7+0.000595*EA*EXP(1500/(273.15+TAIR(JW))))*(1.0D0+CC_LW*CLOUD(JW)*CLOUD(JW))*0.97D0
              !           ELSEIF(I_LWAT==3)THEN  ! Dilley and O'Brien(1998)
...
!************** Ice balance
                ...
                IF (ICE(I)) THEN
                  ...
                  DEL  = 2.0D0
                  J    = 1
                  if(.not.TERM_BY_TERM(JW))then
                  if(Met_Regions)then    !if using TEQ need to compute terms for ice balance - no need to recompute though since it was already used for temperature in water
                         IF(I_LWAT==1)THEN
                             EA = 0.133322*DEXP(2.3026D0*(7.5D0*TDEW(NMet)/(TDEW(NMet)+237.3D0)+0.6609D0))   ! mmHg X 0.133322 ==kPa
                             RANLW(NMet) = 0.0000000562*((273.15D0+TAIR(NMet))**4)*(1.0-(1.0+(465.0*EA)/(273.15+TAIR(NMet)))*EXP(-SQRT(1.2+(3*465*EA)/(273.15+TAIR(NMet)))))*(1.0D0+CC_LW*CLOUD(NMet)*CLOUD(NMet))*0.97D0      !CC_LW=0.0017D0 default
                         ELSEIF(I_LWAT==2)THEN  ! Idso (1981)
                         ...
                         ENDIF
                    !  IF(TAIR(NMet).GE.5.0)THEN
                    !RANLW(NMet) = 5.31D-13*(273.15D0+TAIR(NMet))**6*(1.0D0+CC_LW*CLOUD(NMet)**2)*0.97D0
                    !ELSE
                    !RANLW(NMet) = 5.62D-8*(273.15D0+TAIR(NMet))**4*(1.D0-0.261D0*DEXP(-7.77D-4*TAIR(NMet)**2))*(1.0D0+CC_LW*CLOUD(NMet)**2)*0.97D0
                    !ENDIF
                    RN1=SRON(NMet)/REFL*SHADE(I)*(1.0D0-ALBEDO(JW))*BETAI(JW)+RANLW(NMet)               ! SW 4/19/10 eliminate spurious divsion of SRO by RHOCP
                         ...
                         ENDIF      
                    !IF(TAIR(JW).GE.5.0)THEN
                    !RANLW(JW) = 5.31D-13*(273.15D0+TAIR(JW))**6*(1.0D0+CC_LW*CLOUD(JW)**2)*0.97D0
                    !ELSE
                    !RANLW(JW) = 5.62D-8*(273.15D0+TAIR(JW))**4*(1.D0-0.261D0*DEXP(-7.77D-4*TAIR(JW)**2))*(1.0D0+CC_LW*CLOUD(JW)**2)*0.97D0
                    !ENDIF
                    RN1=SRON(JW)/REFL*SHADE(I)*(1.0D0-ALBEDO(JW))*BETAI(JW)+RANLW(JW)               ! SW 4/19/10 eliminate spurious divsion of SRO by RHOCP  
                  endif
                  else
                    if(Met_Regions)then
                       RN1=SRON(NMet)/REFL*SHADE(I)*(1.0D0-ALBEDO(JW))*BETAI(JW)+RANLW(NMet)
                    else
                      RN1=SRON(JW)/REFL*SHADE(I)*(1.0D0-ALBEDO(JW))*BETAI(JW)+RANLW(JW)    
                    endif
                  ENDIF
...
!************** Ice thickness
                ...
                IF(ICEC(JW) == '    ONWB')THEN
                  ...
                  IceQSS(I) =-IceThicknessChange*BI(KT,I)*DLX(I)*0.917/DLT    ! removal of 0.917 m3 of water for every 1 m3 of ice formed
...
IF(NIT==0)THEN
    CALL INIT_REDUCED_OUTPUT_PLACEHOLDERS
...
!** Temperature transport
    ...
    DO JW=1,NWB
     ...
     !   CALL HORIZONTAL_TRANSPORT
        ...
        DO K=KT,KB(I)     !CONCURRENT(K=KT:KB(I))                                              !DO K=KT,KB(I)
...
CONTAINS
    ...
    SUBROUTINE INIT_REDUCED_OUTPUT_PLACEHOLDERS
      ! Keep reduced-path outputs deterministic without invoking water-quality ENTRY points.
      REAER = 0.0
      KF    = 0.0
      CD    = -99.0
    END SUBROUTINE INIT_REDUCED_OUTPUT_PLACEHOLDERS
