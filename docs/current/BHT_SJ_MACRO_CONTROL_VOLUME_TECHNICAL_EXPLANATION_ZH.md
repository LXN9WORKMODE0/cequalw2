...
- 当前建议值为 `0.0725`，验证支持范围为 `0.070～0.075`。
...
- 这个结果说明方案在当前数据范围内非常有希望，但 `0.0725` 仍然是“工程等效参数”，不是已经识别出的真实河床糙率。
...
> 这是一个已经通过本年度内部验证的工程候选方案，但还不是经过跨年度独立验证的最终生产参数。

因此，后续最正确的动作不是继续追求把 `0.0725` 调到小数点后更多位，而是固定现在的结构和参数，寻找真正独立的数据检验它。只有独立验证暴露出明确、可定位的问题时，才继续增加模型复杂度。
