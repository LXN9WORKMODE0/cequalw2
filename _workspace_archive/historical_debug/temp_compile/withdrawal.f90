     ...
     IF (nnsg==1) THEN                                ! nnsg==1 is gate flow
           ...
           IF (SYSTDG) THEN                          
               IF(GTNAME(jsg)) THEN                   
                  ...
                  if(cac(nn2)=='      ON')CALL  UPDATE_TDGC (1,palt(id),jsg,tavg(js,jb),cavg(js,jb,NN2))  
                  if(cac(ndgp)=='      ON')CALL  UPDATE_TDGC (2,palt(id),jsg,tavg(js,jb),cavg(js,jb,NDGP))
               ELSE
                  call total_dissolved_gas (0,palt(id),nnsg,jsg,tavg(js,jb),cavg(js,jb,NDO))    
                  if(cac(nn2)=='      ON')call total_dissolved_gas (1,palt(id),nnsg,jsg,tavg(js,jb),cavg(js,jb,NN2))     ! n2 GAS
                  if(cac(ndgp)=='      ON')call total_dissolved_gas (2,palt(id),nnsg,jsg,tavg(js,jb),cavg(js,jb,NDGP)) 
               END IF
           ELSE
               call total_dissolved_gas (0,palt(id),nnsg,jsg,tavg(js,jb),cavg(js,jb,NDO))    
               if(cac(nn2)=='      ON')call total_dissolved_gas (1,palt(id),nnsg,jsg,tavg(js,jb),cavg(js,jb,NN2))     ! n2 GAS
...
ENTRY DEALLOCATE_SELECTIVEUSGS
  ...
  deallocate (nout0, nout1, nout2, minfrac1, minfrac2, maxfrac1, maxfrac2, splt2t, splt2e,DYNSF, DYNTF)
RETURN
