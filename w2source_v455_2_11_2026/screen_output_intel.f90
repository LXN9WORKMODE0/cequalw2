...
subroutine W2_DIALOG
  ...
  RESULTLOG = DLGSET    (DLG,STATUS,'Pending ReducedGUI execution')                                   !Display execution status
...
SUBROUTINE RUN_W2 (DLG,CONTROL_NAME,ACTION)
  ...
  SELECT CASE (CONTROL_NAME)
    CASE (RUN)
      ...
      RESULT         =  DLGSET (DLG,STATUS,'Executing ReducedGUI')                                  !Display execution status
    CASE (OUTPUT_DIALOG)
      CALL BLANK_DIALOG  (DLG)
      CALL DATE_AND_TIME (CDATE,CCTIME)
      RESTART_PUSHED = .FALSE.
