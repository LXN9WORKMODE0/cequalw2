...
CONTAINS
    ...
    DO JWP=1,NWB
      DO JBP=BS(JWP),BE(JWP)
        ...
        IF (REUSE_PREDICTOR) THEN
          ...
          VOL_MIN_PRED = TAIL_VOLUME_FROM_STAGE(IUP_PRED, JWP, EL(KB(IUP_PRED)+1,IUP_PRED)+FRONT_H_MIN)
  ...
  REAL(R8) FUNCTION EFFECTIVE_UPSTREAM_INFLOW(JB_IN)
    ...
    IF (UPSTREAM_DOMAIN_LOCK(JB_IN) .AND. TAIL_STAGE_VALID(JB_IN)) THEN
      ...
      VOL_MIN_ACTIVE = TAIL_VOLUME_FROM_STAGE( &
        IUP_ACTIVE, JW_ACTIVE, EL(KB(IUP_ACTIVE)+1,IUP_ACTIVE)+FRONT_H_MIN)
