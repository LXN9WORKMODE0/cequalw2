...
SUBROUTINE KINETICS
  ...
  USE MAIN, ONLY:NPBALC, EPIPHYTON_CALC, BOD_CALC, ALG_CALC, BOD_CALCN, BOD_CALCP, PO4_CALC, N_CALC, DSI_CALC, STANDING_BIOMASS_DECAY, NH3_DER, CDWBC,KF_NH4_SR,KF_NH4_SD,KF_PO4_SR,KF_PO4_SD, ORGC_CALC, HCO3_DER, CO3_DER, CBODU_DER,TOTSS_DER,O2DG_DER,TURB_DER,SECCHI_DER, CHLA_DER, GAS_TRANSFER_UPDATE, NFEII, NFEOOH, NMNII, NMNO2
...
! Type declarations
  ...
  REAL                                :: ALGP,ALGN,ZOOP,ZOON,XX,KHCO2
  ...
  real                                :: ticch4, DK1_BACT, SET_BACT, PHOTO_BACT, SODTRMDDO3, OMTRMDO3                                              !h2sex,ch4ex,ticch4
...
ENTRY TEMPERATURE_RATES
  DO I=IU,ID
    DO K=KT,KB(I)
      ...
      END DO
      ! MACROPHYTEC block removed (DO M=1,NMC)
#ifdef ZOOPLANKTONC
...
ENTRY LABILE_DOM
  ...
  DO I=IU,ID
    DO K=KT,KB(I)
      ...
      LDOMSS(K,I) = LDOMAP(K,I)+LDOMEP(K,I)-LDOMD(K,I)-LRDOMD(K,I)+LPOMHD(K,I)
#ifdef MACROPHYTEC
     +LDOMMAC(K,I)
#endif
...
#endif
#ifdef ZOOPLANKTONC
  LPZOOIN(:,IU:ID)=0.0;LPZOOOUT(:,IU:ID)=0.0   ! cb 5/19/06
#endif
  DO I=IU,ID
    DO K=KT,KB(I)
      DO JA=1,NAL
      ...
      ENDIF
#endif
      LPOMSS(K,I) = LPOMAP(K,I)+LPOMEP(K,I)-LPOMD(K,I)+LPOMNS(K,I)-LRPOMD(K,I)-LPOMHD(K,I)
#ifdef MACROPHYTEC
     +LPOMMAC(K,I)
#endif
#ifdef ZOOPLANKTONC
...
ENTRY REFRACTORY_POM
  ...
  DO I=IU,ID
    DO K=KT,KB(I)
      ...
      RPOMSS(K,I) = LRPOMD(K,I)+RPOMNS(K,I)-RPOMD(K,I)-RPOMHD(K,I)
#ifdef MACROPHYTEC
     +RPOMMAC(K,I)
#endif
...
ENTRY DISSOLVED_OXYGEN
  ...
  DO I=IU,ID
    ...
    DO K=KT,KB(I)
	 ...
	 ENDIF
#endif
    DOSS(K,I)  =  DOAP(K,I)+DOEP(K,I)-DOAR(K,I)-DOER(K,I)-DOOM(K,I)-DONIT(K,I)-DOSOD(K,I)-DOSED(K,I)+doch4(k,i)+doh2s(k,i)+dofe2(k,i)+doMn2(k,i)
#ifdef ZOOPLANKTONC
     DOSS(K,I) = DOSS(K,I)+DOMP(K,I)-DOMR(K,I)-DOZR(K,I)
#endif
      ...
      ENDIF
            ...
            IF(STANDING_BIOMASS_DECAY)THEN  ! SW 5/26/15
               TICSS(K,I) = TICAP(K,I)+TICEP(K,I)+SEDDC(K,I)+ORGC(JW)*(LPOMD(K,I)+RPOMD(K,I)+LDOMD(K,I)+RDOMD(K,I))                          &
                   +CO2R(JW)*SODD(K,I)*DO3(K,I)+TICBOD(K,I)+ticch4+sedd1(k,i)*cbiom(jw) + sedd2(k,i)*cbiom(jw)
#ifdef MACROPHYTEC
               TICSS(K,I) = TICSS(K,I)+TICMC(K,I)+TICZR(K,I)
#endif

            ELSE
                TICSS(K,I) = TICAP(K,I)+TICEP(K,I)+SEDDC(K,I)+ORGC(JW)*(LPOMD(K,I)+RPOMD(K,I)+LDOMD(K,I)+RDOMD(K,I))                          &
              +CO2R(JW)*SODD(K,I)*DO3(K,I)+TICBOD(K,I)+ticch4
#ifdef MACROPHYTEC
                TICSS(K,I) = TICSS(K,I)+TICMC(K,I)+TICZR(K,I)
#endif
            ENDIF
...
! Nitrogen has an atomic mass of 14.00674. These numbers account for the factor of 50.044/14.00674 used below.
 ...
 DO I=IU,ID
   DO K=KT,KB(I)
     ...
     ALKSS(K,I) = (50.044/14.00674)*( 14./16.*(NH4AP(K,I)+NH4EP(K,I))+ 18./16.*(NO3AG(K,I)+NO3EG(K,I))- 2.*NH4D(K,I) + NO3D(K,I) + NO3SED(K,I)*(1-FNO3SED(JW)) )
#ifdef ZOOPLANKTONC
     ALKSS(K,I) = ALKSS(K,I)+(50.044/14.00674)*( 14./16.*NH4ZR(K,I))
#endif
#ifdef MACROPHYTEC
     ALKSS(K,I) = ALKSS(K,I)+(50.044/14.00674)*( 14./16.*(NH4MR(K,I)-NH4MG(K,I)))
#endif
       else
