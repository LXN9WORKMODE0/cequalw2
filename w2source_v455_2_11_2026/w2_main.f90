...
CONTAINS
  ...
  SUBROUTINE RUN_TAIL_INTERFACE_ITERATION(JB_IN, JW_IN)
    ...
    LOGICAL :: Q_SECANT_ENABLED, Q_UPDATE_HELD
    CHARACTER(32), SAVE :: Q_UPDATE_MODE = 'SECANT'
    LOGICAL, SAVE :: Q_UPDATE_MODE_INIT = .FALSE.

    IF (.NOT. TAIL_COUPLED(JB_IN)) RETURN
    IF (TAIL_UPSEG(JB_IN) <= 0 .OR. TAIL_DNSEG(JB_IN) <= 0) RETURN
    IF (.NOT. Q_UPDATE_MODE_INIT) THEN
      CALL GET_ENVIRONMENT_VARIABLE('W2_TAIL_Q_UPDATE_MODE', Q_UPDATE_MODE, STATUS=ENV_STATUS)
      IF (ENV_STATUS /= 0 .OR. LEN_TRIM(Q_UPDATE_MODE) == 0) Q_UPDATE_MODE = 'SECANT'
      Q_UPDATE_MODE = ADJUSTL(Q_UPDATE_MODE)
      Q_UPDATE_MODE_INIT = .TRUE.
      WARNING_OPEN = .TRUE.
      WRITE (WRN,'(A,1X,A)') '[V23_Q_UPDATE_MODE]', 'MODE='//TRIM(Q_UPDATE_MODE)
    END IF
    Q_UPDATE_HELD = Q_UPDATE_MODE(1:4) == 'HOLD' .OR. Q_UPDATE_MODE(1:4) == 'hold'
    Q_SECANT_ENABLED = .NOT. Q_UPDATE_HELD .AND. Q_UPDATE_MODE(1:5) /= 'RELAX' .AND. Q_UPDATE_MODE(1:5) /= 'relax'
