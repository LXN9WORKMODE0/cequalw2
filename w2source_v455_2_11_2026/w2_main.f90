...
PROGRAM W2_MAIN
 ...
 ! include "omp_lib.h"      ! OPENMP directive to adjust the # of processors TOGGLE FOR DEBUG
  ...
  REAL(R8)      :: TAIL_FLUX_GAP_DIAG, TAIL_COMBINED_RESID_DIAG
  REAL(R8)      :: TAIL_ACCEPT_VOL_MIN, TAIL_ACCEPT_QMAX, TAIL_ACCEPT_DV_UP, TAIL_ACCEPT_DV_DN
  REAL(R8)      :: TAIL_FULL_STORAGE_RATE, TAIL_FULL_STORAGE_RESID, TAIL_STORAGE_RATE_GAP
  REAL(R8)      :: TAIL_FULL_STORAGE_RAW, TAIL_FULL_STORAGE_ROUNDOFF
  REAL(R8)      :: TAIL_SEGMENT_VOL_SUM, TAIL_SEGMENT_VOL_GAP
  REAL(R8)      :: TAIL_LOCAL_STAGE_DN, TAIL_LOCAL_DX, TAIL_LOCAL_QTARGET
  REAL(R8)      :: TAIL_MACRO_STAGE, TAIL_MACRO_DX, TAIL_MACRO_QTARGET
  REAL(R8)      :: TAIL_MACRO_QAGG, TAIL_MACRO_RCOEFF, TAIL_MACRO_RFRIC, TAIL_MACRO_RVEL
  INTEGER       :: TAIL_LOCAL_ISEG, TAIL_LOCAL_NEXT_ISEG, TAIL_MACRO_ISEG, TAIL_MACRO_IT
  LOGICAL       :: CSVFORMAT, LOG_INTERFACE_MASS, TAIL_MACRO_VALID
 ...
 !!$OMP END PARALLEL DO
    DO JW=1,NWB
      ...
      IF(BR_INACTIVE(JB))CYCLE
        ...
        TAIL_RESERVOIR_Q_USED(JB) = QINJB_ACTIVE
...
!***********************************************************************************************************************************
    ...
    END IF
    ! Recompute the corrector from the same beginning-of-step tail state as
    ! the predictor.  Advancing from the predicted state would integrate the
    ! tail storage twice during one accepted W2 timestep.
    CALL RESTORE_TAIL_DYNAMIC_STATE()
    DO JW=1,NWB
      DO JB=BS(JW),BE(JW)
        IF (.NOT. TAIL_COUPLED(JB)) CYCLE
        IF (TAIL_UPSEG(JB) <= 0 .OR. TAIL_DNSEG(JB) <= 0) CYCLE
        CALL COMMIT_TAIL_INTERFACE(JB, JW)
        WARNING_OPEN = .TRUE.
        ...
        END IF
        TAIL_FLUX_GAP_DIAG = TAIL_RESERVOIR_Q_USED(JB)-TAIL_Q_LINK(JB)
        TAIL_COMBINED_RESID_DIAG = TAIL_MASS_RESIDUAL(JB)+TAIL_FLUX_GAP_DIAG
        LOG_INTERFACE_MASS = NIT <= 10 .OR. MOD(NIT,500) == 0
        IF (ABS(TAIL_MASS_RESIDUAL(JB)) > TAIL_MASS_RESIDUAL_MAX(JB)) LOG_INTERFACE_MASS = .TRUE.
        IF (ABS(TAIL_FLUX_GAP_DIAG) > TAIL_FLUX_GAP_MAX(JB)) LOG_INTERFACE_MASS = .TRUE.
        IF (ABS(TAIL_COMBINED_RESID_DIAG) > TAIL_COMBINED_MASS_RESIDUAL_MAX(JB)) LOG_INTERFACE_MASS = .TRUE.
        IF (ABS(TAIL_SEGMENT_Q_LOSS(JB)) > TAIL_SEGMENT_Q_LOSS_MAX(JB)) LOG_INTERFACE_MASS = .TRUE.
        TAIL_MASS_RESIDUAL_MAX(JB) = MAX(TAIL_MASS_RESIDUAL_MAX(JB), ABS(TAIL_MASS_RESIDUAL(JB)))
        TAIL_FLUX_GAP_MAX(JB) = MAX(TAIL_FLUX_GAP_MAX(JB), ABS(TAIL_FLUX_GAP_DIAG))
        TAIL_COMBINED_MASS_RESIDUAL_MAX(JB) = MAX(TAIL_COMBINED_MASS_RESIDUAL_MAX(JB), ABS(TAIL_COMBINED_RESID_DIAG))
        TAIL_SEGMENT_Q_LOSS_MAX(JB) = MAX(TAIL_SEGMENT_Q_LOSS_MAX(JB), ABS(TAIL_SEGMENT_Q_LOSS(JB)))
        IF (LOG_INTERFACE_MASS) THEN
          WRITE (WRN,'(A,1X,A,I0,8(1X,A,ES12.4))') '[V24_INTERFACE_MASS]', 'JB=',JB, &
            'QPHYS=',TAIL_Q_INFLOW(JB), 'QIFACE=',TAIL_Q_LINK(JB), 'QRES=',TAIL_RESERVOIR_Q_USED(JB), &
            'DSTORAGE=',TAIL_STORAGE_CHANGE_RATE(JB), 'RTAIL=',TAIL_MASS_RESIDUAL(JB), &
            'RFLUX=',TAIL_FLUX_GAP_DIAG, 'RCOMB=',TAIL_COMBINED_RESID_DIAG, 'SEGLOSS=',TAIL_SEGMENT_Q_LOSS(JB)
        END IF
...
!** Accepted tail-state diagnostics (after all autostep rollback checks)

    DO JW=1,NWB
      DO JB=BS(JW),BE(JW)
        IF (.NOT. TAIL_COUPLED(JB) .OR. .NOT. TAIL_STAGE_VALID(JB)) CYCLE
        IF (TAIL_REACH_INITIALIZED_SAVE(JB)) THEN
          TAIL_FULL_STORAGE_RATE = (TAIL_STORAGE_VOL(JB)-TAIL_STORAGE_VOL_SAVE(JB))/MAX(ABS(DLT),1.0D-12)
          TAIL_FULL_STORAGE_RAW = TAIL_FULL_STORAGE_RATE-(TAIL_Q_INFLOW(JB)-TAIL_Q_LINK(JB))
          TAIL_FULL_STORAGE_ROUNDOFF = 0.5D0*MAX( &
            SPACING(TAIL_STORAGE_VOL_SAVE(JB)),SPACING(TAIL_STORAGE_VOL(JB)))/MAX(ABS(DLT),1.0D-12)
          TAIL_FULL_STORAGE_RESID = TAIL_CONTINUITY_RESIDUAL(TAIL_FULL_STORAGE_RAW, &
            TAIL_STORAGE_VOL_SAVE(JB), TAIL_STORAGE_VOL(JB), DLT)
          TAIL_STORAGE_RATE_GAP = TAIL_FULL_STORAGE_RATE-TAIL_STORAGE_CHANGE_RATE(JB)
          WARNING_OPEN = .TRUE.
          WRITE (WRN,'(A,1X,A,I0,7(1X,A,ES12.4))') '[V31_SINGLE_STEP]', 'JB=',JB, &
            'VPRE=',TAIL_STORAGE_VOL_SAVE(JB), 'VFINAL=',TAIL_STORAGE_VOL(JB), &
            'RATE=',TAIL_FULL_STORAGE_RATE, 'RTAIL=',TAIL_FULL_STORAGE_RESID, 'RATEGAP=',TAIL_STORAGE_RATE_GAP, &
            'RRAW=',TAIL_FULL_STORAGE_RAW, 'RBOUND=',TAIL_FULL_STORAGE_ROUNDOFF
        END IF
        TAIL_SEGMENT_VOL_SUM = SUM(TAIL_VOL_SEG(1:TAIL_DOMAIN_NSEG(JB),JB))
        TAIL_SEGMENT_VOL_GAP = TAIL_STORAGE_VOL(JB)-TAIL_SEGMENT_VOL_SUM
        WARNING_OPEN = .TRUE.
        WRITE (WRN,'(A,1X,A,I0,3(1X,A,ES12.4))') '[V32_SEGMENT_VOLUME]', 'JB=',JB, &
          'VTOTAL=',TAIL_STORAGE_VOL(JB), 'VSEG=',TAIL_SEGMENT_VOL_SUM, 'VGAP=',TAIL_SEGMENT_VOL_GAP
        ! V33 is a shadow preflight only.  It evaluates the current local Manning
        ! closure on every cell interface without committing flux or state.
        DO IS=1,TAIL_DOMAIN_NSEG(JB)
          TAIL_LOCAL_ISEG = TAIL_DOMAIN_US(JB)+IS-1
          IF (IS < TAIL_DOMAIN_NSEG(JB)) THEN
            TAIL_LOCAL_NEXT_ISEG = TAIL_LOCAL_ISEG+1
            TAIL_LOCAL_STAGE_DN = TAIL_STAGE_SEG(IS+1,JB)
          ELSE
            TAIL_LOCAL_NEXT_ISEG = TAIL_DNSEG(JB)
            TAIL_LOCAL_STAGE_DN = TAIL_WSE_DN(JB)
          END IF
          TAIL_LOCAL_DX = MAX(0.5D0*(DLX(TAIL_LOCAL_ISEG)+DLX(TAIL_LOCAL_NEXT_ISEG)),1.0D0)
          CALL COMPUTE_TAIL_LOCAL_LINK_FLOW(TAIL_LOCAL_ISEG, JW, TAIL_STAGE_SEG(IS,JB), &
            TAIL_LOCAL_STAGE_DN, TAIL_LOCAL_DX, TAIL_LOCAL_QTARGET)
          WRITE (WRN,'(A,1X,A,I0,1X,A,I0,1X,A,I0,6(1X,A,ES12.4))') '[V33_LOCAL_TARGET]', &
            'JB=',JB, 'IS=',IS, 'ISEG=',TAIL_LOCAL_ISEG, 'QCOMMIT=',TAIL_Q_LINK(JB), &
            'QTARGET=',TAIL_LOCAL_QTARGET, 'WUP=',TAIL_STAGE_SEG(IS,JB), 'WDN=',TAIL_LOCAL_STAGE_DN, &
            'DX=',TAIL_LOCAL_DX, 'STORAGE=',TAIL_VOL_SEG(IS,JB)
        END DO
        ! V37 is observation-anchored shadow evidence only.  The optional
        ! interface does not own volume and cannot change committed flow.
        TAIL_MACRO_ISEG = TAIL_MACRO_INTERFACE_SEG(JB)
        IF (TAIL_MACRO_ISEG > TAIL_UPSEG(JB) .AND. TAIL_MACRO_ISEG <= DS(JB)) THEN
          IF (TAIL_MACRO_ISEG < TAIL_COUPLE_SEG(JB)) THEN
            TAIL_MACRO_STAGE = TAIL_STAGE_SEG(TAIL_MACRO_ISEG-TAIL_DOMAIN_US(JB)+1,JB)
          ELSE
            TAIL_MACRO_STAGE = ELWS(TAIL_MACRO_ISEG)
          END IF
          TAIL_MACRO_DX = 0.0D0
          DO TAIL_MACRO_IT=TAIL_UPSEG(JB),TAIL_MACRO_ISEG-1
            TAIL_MACRO_DX = TAIL_MACRO_DX + 0.5D0*(DLX(TAIL_MACRO_IT)+DLX(TAIL_MACRO_IT+1))
          END DO
          CALL COMPUTE_TAIL_LOCAL_LINK_FLOW(TAIL_UPSEG(JB), JW, TAIL_WSE_UP(JB), &
            TAIL_MACRO_STAGE, TAIL_MACRO_DX, TAIL_MACRO_QTARGET)
          CALL COMPUTE_TAIL_MACRO_LINK_FLOW(JB, JW, TAIL_UPSEG(JB), TAIL_MACRO_ISEG, &
            TAIL_WSE_UP(JB), TAIL_MACRO_STAGE, TAIL_MACRO_QAGG, TAIL_MACRO_RCOEFF, &
            TAIL_MACRO_RFRIC, TAIL_MACRO_RVEL, TAIL_MACRO_VALID)
          WRITE (WRN,'(A,1X,A,I0,1X,A,I0,11(1X,A,ES12.4),1X,A,L1)') '[V37_MACRO_TARGET]', &
            'JB=',JB, 'ISEG=',TAIL_MACRO_ISEG, 'JDAY=',JDAY, 'QPHYS=',QIN(JB), 'QCOMMIT=',TAIL_Q_LINK(JB), &
            'QTARGET=',TAIL_MACRO_QTARGET, 'QAGG=',TAIL_MACRO_QAGG, 'WUP=',TAIL_WSE_UP(JB), &
            'WIFACE=',TAIL_MACRO_STAGE, 'DX=',TAIL_MACRO_DX, 'RCOEFF=',TAIL_MACRO_RCOEFF, &
            'RFRIC=',TAIL_MACRO_RFRIC, 'RVEL=',TAIL_MACRO_RVEL, 'VALID=',TAIL_MACRO_VALID
        END IF
        IF (TAIL_MACRO_ACTIVE_SEG(JB) > 0) THEN
          WRITE (WRN,'(A,1X,A,I0,1X,A,I0,7(1X,A,ES12.4))') '[V38_ACTIVE_MACRO]', &
            'JB=',JB, 'ISEG=',TAIL_MACRO_ACTIVE_SEG(JB), 'JDAY=',JDAY, &
            'NEFF=',TAIL_MACRO_ACTIVE_MANNING(JB), 'QPHYS=',QIN(JB), 'QCOMMIT=',TAIL_Q_LINK(JB), &
            'QTARGET=',TAIL_Q_TARGET(JB), 'WUP=',TAIL_WSE_UP(JB), 'WDN=',TAIL_WSE_DN(JB)
        END IF
        TAIL_ACCEPT_VOL_MIN = TAIL_MIN_PROFILE_VOLUME(JB, JW, TAIL_WSE_DN(JB))
        TAIL_ACCEPT_QMAX = MAX(QIN(JB),0.0D0) + &
          MAX(TAIL_STORAGE_VOL(JB)-TAIL_ACCEPT_VOL_MIN,0.0D0)/MAX(ABS(DLT),1.0D-12)
        TAIL_ACCEPT_DV_UP = (TAIL_PROFILE_VOLUME(JB, JW, TAIL_WSE_UP(JB)+0.01D0, TAIL_WSE_DN(JB)) - &
          TAIL_STORAGE_VOL(JB))/0.01D0
        TAIL_ACCEPT_DV_DN = (TAIL_PROFILE_VOLUME(JB, JW, TAIL_WSE_UP(JB), TAIL_WSE_DN(JB)+0.01D0) - &
          TAIL_STORAGE_VOL(JB))/0.01D0
        WARNING_OPEN = .TRUE.
        WRITE (WRN,'(A,1X,A,I0,1X,A,F16.9,10(1X,A,ES14.6),1X,A,L1)') &
          '[V29_ACCEPTED_TAIL]', 'JB=',JB, &
          'JDAY=',JDAY, 'DT=',DLT, 'QPHYS=',QIN(JB), 'QSTATE=',TAIL_Q_LINK(JB), &
          'QTARGET=',TAIL_Q_TARGET(JB), 'QMAX=',TAIL_ACCEPT_QMAX, 'STORAGE=',TAIL_STORAGE_VOL(JB), &
          'WUP=',TAIL_WSE_UP(JB), 'WDN=',TAIL_WSE_DN(JB), &
          'DVUP=',TAIL_ACCEPT_DV_UP, 'DVDN=',TAIL_ACCEPT_DV_DN, &
          'CAP=',TAIL_Q_LINK(JB) >= TAIL_ACCEPT_QMAX-1.0D-6
      END DO
    END DO
...
CONTAINS
  ...
  SUBROUTINE COMMIT_TAIL_INTERFACE(JB_IN, JW_IN)
    INTEGER, INTENT(IN) :: JB_IN, JW_IN
    REAL(R8) :: ETA_TARGET, ETA_PREDICTED, Q_PREDICTED, Q_COMMIT
    LOGICAL, SAVE :: CONSERVATIVE_MODE_LOGGED = .FALSE.

    IF (.NOT. TAIL_COUPLED(JB_IN)) RETURN
    IF (TAIL_UPSEG(JB_IN) <= 0 .OR. TAIL_DNSEG(JB_IN) <= 0) RETURN

    IF (.NOT. CONSERVATIVE_MODE_LOGGED) THEN
      WARNING_OPEN = .TRUE.
      WRITE (WRN,'(A,1X,A)') '[V23_Q_UPDATE_MODE]', 'MODE=CONSERVATIVE'
      CONSERVATIVE_MODE_LOGGED = .TRUE.
    END IF

    ETA_TARGET = ELWS(TAIL_DNSEG(JB_IN))
    ETA_PREDICTED = TAIL_IFACE_ETA_PRED(JB_IN)
    Q_PREDICTED = MAX(TAIL_IFACE_Q_PRED(JB_IN), 0.0D0)
    Q_COMMIT = MAX(TAIL_RESERVOIR_Q_USED(JB_IN), 0.0D0)

    TAIL_IFACE_ETA_PREV(JB_IN) = ETA_PREDICTED
    TAIL_IFACE_Q_PREV(JB_IN) = Q_PREDICTED
    TAIL_IFACE_DETA_STEP(JB_IN) = ETA_TARGET-ETA_PREDICTED
    TAIL_IFACE_DQ_STEP(JB_IN) = Q_COMMIT-Q_PREDICTED

    CALL UPDATE_TAIL_STAGE(JB_IN, JW_IN, ETA_TARGET, Q_COMMIT)

    TAIL_WSE_DN(JB_IN) = ETA_TARGET
    TAIL_IFACE_ETA_GUESS(JB_IN) = TAIL_WSE_DN(JB_IN)
    TAIL_IFACE_Q_GUESS(JB_IN) = TAIL_Q_LINK(JB_IN)
    TAIL_IFACE_ETA_PRED(JB_IN) = TAIL_IFACE_ETA_GUESS(JB_IN)
    TAIL_IFACE_Q_PRED(JB_IN) = TAIL_IFACE_Q_GUESS(JB_IN)
    TAIL_IFACE_ETA_CORR(JB_IN) = TAIL_IFACE_ETA_GUESS(JB_IN)
    TAIL_IFACE_Q_CORR(JB_IN) = TAIL_IFACE_Q_GUESS(JB_IN)
    TAIL_IFACE_DETA(JB_IN) = TAIL_IFACE_ETA_CORR(JB_IN)-TAIL_IFACE_ETA_PRED(JB_IN)
    TAIL_IFACE_DQ(JB_IN) = TAIL_IFACE_Q_CORR(JB_IN)-TAIL_IFACE_Q_PRED(JB_IN)
    TAIL_IFACE_MAX_DETA(JB_IN) = MAX(TAIL_IFACE_MAX_DETA(JB_IN), ABS(TAIL_IFACE_DETA_STEP(JB_IN)))
    TAIL_IFACE_MAX_DQ(JB_IN) = MAX(TAIL_IFACE_MAX_DQ(JB_IN), ABS(TAIL_IFACE_DQ_STEP(JB_IN)))
    TAIL_IFACE_ITER_COUNT(JB_IN) = 1
    TAIL_IFACE_CONVERGED(JB_IN) = ABS(TAIL_Q_LINK(JB_IN)-Q_COMMIT) < TAIL_IFACE_Q_TOL
    TAIL_IFACE_RELAX(JB_IN) = 1.0D0

    IF (NIT <= 10 .OR. MOD(NIT,500) == 0 .OR. .NOT. TAIL_IFACE_CONVERGED(JB_IN)) THEN
      WARNING_OPEN = .TRUE.
      WRITE (WRN,'(A,1X,A,I0,1X,A,I0,1X,A,ES12.4,1X,A,ES12.4,1X,A,F0.3,1X,A,L1)') '[V20_INTERFACE_ITER]', &
        'JB=',JB_IN, 'ITER=',1, 'DETA=',TAIL_IFACE_DETA(JB_IN), 'DQ=',TAIL_IFACE_DQ(JB_IN), &
        'RELAX=',TAIL_IFACE_RELAX(JB_IN), 'CONV=',TAIL_IFACE_CONVERGED(JB_IN)
      WRITE (WRN,'(A,1X,A,I0,1X,A,I0,1X,A,ES12.4,1X,A,ES12.4,1X,A,F0.3,1X,A,F0.3)') '[V21_INTERFACE_SOLVE]', &
        'JB=',JB_IN, 'ITER=',1, 'DETA_STEP=',TAIL_IFACE_DETA_STEP(JB_IN), 'DQ_STEP=',TAIL_IFACE_DQ_STEP(JB_IN), &
        'ETA=',TAIL_IFACE_ETA_GUESS(JB_IN), 'Q=',TAIL_IFACE_Q_GUESS(JB_IN)
    END IF
    IF (NIT <= 10 .OR. MOD(NIT,500) == 0 .OR. .NOT. TAIL_IFACE_CONVERGED(JB_IN)) THEN
      WRITE (WRN,'(A,1X,A,I0,5(1X,A,ES12.4),1X,A,L1)') '[V24_INTERFACE_COMMIT]', 'JB=',JB_IN, &
        'QRES=',Q_COMMIT, 'QCOMMIT=',TAIL_Q_LINK(JB_IN), 'ETA_PREV=',ETA_PREDICTED, &
        'ETA_COMMIT=',TAIL_WSE_DN(JB_IN), 'DETA_APPLIED=',TAIL_IFACE_DETA_STEP(JB_IN), &
        'EVALUATED=',TAIL_IFACE_CONVERGED(JB_IN)
    END IF
  END SUBROUTINE COMMIT_TAIL_INTERFACE

  SUBROUTINE RUN_TAIL_PREDICTOR()
    INTEGER :: JWP, JBP
    LOGICAL :: ANY_PREDICT_COMPUTED
  ...
  SUBROUTINE INITIALIZE_TAIL_SEGMENT_STATE(JB_IN)
    ...
    DO IS=1,NSEG
      ...
      FRAC = TAIL_PROFILE_FRACTION(JB_IN, ISEG)
  ...
  SUBROUTINE UPDATE_ONE_TAIL_SEGMENT(JB_IN, JW_IN, IS, QUP, QDN)
    ...
    REAL(R8) :: TARGET_STAGE
    LOGICAL :: OK

    ISEG = TAIL_DOMAIN_US(JB_IN) + IS - 1
    BED_ELEV = EL(KB(ISEG)+1,ISEG)
    STAGE_UP = TAIL_STAGE_SEG(IS,JB_IN)
    ...
    ! Until segment continuity is solved explicitly, segment state is a
    ! mass-conservative derived view of the single committed interface flux.
    QDN = MAX(QUP, 0.0D0)
    TARGET_STAGE = MAX(BED_ELEV+FRONT_H_MIN, STAGE_UP)
    TAIL_STAGE_SEG(IS,JB_IN) = TARGET_STAGE
    TAIL_DEPTH_SEG(IS,JB_IN) = MAX(TAIL_STAGE_SEG(IS,JB_IN)-BED_ELEV, FRONT_H_MIN)
    TAIL_AREA_SEG(IS,JB_IN) = MAX(AREA_LOCAL, 1.0D-6)
    TAIL_HRAD_SEG(IS,JB_IN) = MAX(HRAD_LOCAL, 1.0D-6)
  ...
  SUBROUTINE UPDATE_TAIL_SEGMENT_TRANSITION(JB_IN, JW_IN, IS)
    ...
    IF (IS < NSEG) THEN
      ...
      SLOPE_LOCAL = MAX((TAIL_STAGE_SEG(IS,JB_IN)-TAIL_STAGE_SEG(IS+1,JB_IN))/ &
        MAX(0.5D0*(DLX(ISEG)+DLX(NEXT_ISEG)), 1.0D0), 0.0D0)
    ELSE
      NEXT_ISEG = TAIL_DNSEG(JB_IN)
      DEPTH_NEXT = MAX(TAIL_WSE_DN(JB_IN)-EL(KB(NEXT_ISEG)+1,NEXT_ISEG), FRONT_H_MIN)
      SLOPE_LOCAL = MAX((TAIL_STAGE_SEG(IS,JB_IN)-TAIL_WSE_DN(JB_IN))/ &
        MAX(0.5D0*(DLX(ISEG)+DLX(NEXT_ISEG)), 1.0D0), 0.0D0)
    END IF
  ...
  SUBROUTINE UPDATE_TAIL_STAGE(JB_IN, JW_IN, ETA_INTERFACE_GUESS, Q_INTERFACE_COMMIT)
    INTEGER, INTENT(IN) :: JB_IN, JW_IN
    REAL(R8), INTENT(IN), OPTIONAL :: ETA_INTERFACE_GUESS
    REAL(R8), INTENT(IN), OPTIONAL :: Q_INTERFACE_COMMIT
    ...
    REAL(R8) :: VOL_TARGET, VOL_MIN, STORAGE_OLD, QALPHA, DEPTH_STAGE, QOUT_BEFORE_SEGMENTS
    REAL(R8) :: Q_AVAILABLE_MAX, QSTATE_UNCAPPED
    REAL(R8) :: PROFILE_VOL_CHECK, PROFILE_VOL_GAP
    REAL(R8) :: PROFILE_RCOEFF, PROFILE_RFRIC, PROFILE_RVEL
    LOGICAL :: SOLVED, ACTIVE_MACRO, PROFILE_OK
    ...
    QABS = ABS(QC(IDN_SEG))
    ACTIVE_MACRO = TAIL_MACRO_ACTIVE_SEG(JB_IN) == IDN_SEG .AND. &
      TAIL_MACRO_ACTIVE_MANNING(JB_IN) > 0.0D0
    IF (ACTIVE_MACRO .AND. .NOT. TAIL_Q_STATE_INITIALIZED(JB_IN)) QABS = QPHYS

    IF (QABS <= 1.0D-6) THEN
      WSE_HYD = MAX(TAIL_WSE_DN(JB_IN), EL(KB(IUP_SEG)+1,IUP_SEG)+FRONT_H_MIN)
      TAIL_STAGE_MODE(JB_IN) = 1
    ELSEIF (ACTIVE_MACRO) THEN
      CALL SOLVE_TAIL_PROFILE_STAGE(JB_IN, JW_IN, IUP_SEG, IDN_SEG, QABS, TAIL_WSE_DN(JB_IN), &
        TAIL_MACRO_ACTIVE_MANNING(JB_IN), WSE_HYD, SOLVED)
      IF (.NOT. SOLVED) THEN
        WRITE(*,'(A,I0)') 'Unable to solve active macro profile stage for branch ', JB_IN
        STOP 2
      END IF
      TAIL_STAGE_MODE(JB_IN) = 4
    ELSE
    ...
    IF (.NOT. TAIL_REACH_INITIALIZED(JB_IN)) THEN
      TAIL_STORAGE_VOL(JB_IN) = TAIL_PROFILE_VOLUME(JB_IN, JW_IN, WSE_HYD, TAIL_WSE_DN(JB_IN))
      TAIL_REACH_INITIALIZED(JB_IN) = .TRUE.
    END IF

    VOL_MIN = TAIL_MIN_PROFILE_VOLUME(JB_IN, JW_IN, TAIL_WSE_DN(JB_IN))
    IF (ACTIVE_MACRO) THEN
      CALL COMPUTE_TAIL_PROFILE_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, &
        MAX(TAIL_WSE_UP(JB_IN), WSE_HYD), TAIL_WSE_DN(JB_IN), &
        TAIL_MACRO_ACTIVE_MANNING(JB_IN), QLINK_RAW, PROFILE_RCOEFF, PROFILE_RFRIC, PROFILE_RVEL, PROFILE_OK)
      IF (.NOT. PROFILE_OK) THEN
        WRITE(*,'(A,I0)') 'Unable to evaluate active macro profile flow for branch ', JB_IN
        STOP 2
      END IF
    ELSE
      CALL COMPUTE_TAIL_LINK_FLOW(IUP_SEG, IDN_SEG, JB_IN, JW_IN, &
        MAX(TAIL_WSE_UP(JB_IN), WSE_HYD), TAIL_WSE_DN(JB_IN), QLINK_RAW)
    END IF
    QTARGET_OLD = MAX(QLINK_RAW, 0.0D0)
    TAIL_Q_TARGET(JB_IN) = QTARGET_OLD
    STORAGE_OLD = TAIL_STORAGE_VOL(JB_IN)
    Q_AVAILABLE_MAX = QPHYS + MAX(STORAGE_OLD-VOL_MIN, 0.0D0)/MAX(ABS(DLT), 1.0D-12)
    ...
    END IF
    IF (PRESENT(Q_INTERFACE_COMMIT)) THEN
      QSTATE_NEW = MAX(Q_INTERFACE_COMMIT, 0.0D0)
      QALPHA = 0.0D0
    ELSE
    ...
    CALL INVERT_TAIL_PROFILE_STORAGE(JB_IN, JW_IN, VOL_TARGET, TAIL_WSE_DN(JB_IN), WSE_STORAGE, ACTIVE_MACRO)
    WSE_SOLVED = MAX(WSE_STORAGE, TAIL_WSE_DN(JB_IN))
    IF (ACTIVE_MACRO) THEN
      CALL COMPUTE_TAIL_PROFILE_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, WSE_SOLVED, &
        TAIL_WSE_DN(JB_IN), TAIL_MACRO_ACTIVE_MANNING(JB_IN), QLINK_RAW, &
        PROFILE_RCOEFF, PROFILE_RFRIC, PROFILE_RVEL, PROFILE_OK)
      IF (.NOT. PROFILE_OK) THEN
        WRITE(*,'(A,I0)') 'Unable to update active macro profile flow for branch ', JB_IN
        STOP 2
      END IF
    ELSE
    ...
    IF (.NOT. PRESENT(Q_INTERFACE_COMMIT)) THEN
      CALL ADVANCE_TAIL_Q_STATE(JB_IN, IUP_SEG, JW_IN, WSE_SOLVED, QSTATE_PREV, TAIL_Q_TARGET(JB_IN), QSTATE_NEW, QALPHA)
    END IF
    QSTATE_UNCAPPED = QSTATE_NEW
    QSTATE_NEW = MIN(QSTATE_NEW, Q_AVAILABLE_MAX)
    IF (QSTATE_UNCAPPED > QSTATE_NEW+1.0D-9 .AND. (NIT <= 10 .OR. MOD(NIT,500) == 0)) THEN
      WARNING_OPEN = .TRUE.
      WRITE (WRN,'(A,1X,A,I0,3(1X,A,ES12.4))') '[V24_Q_AVAILABLE]', 'JB=',JB_IN, &
        'QRAW=',QSTATE_UNCAPPED, 'QMAX=',Q_AVAILABLE_MAX, 'QCOMMIT=',QSTATE_NEW
    END IF
    ...
    QOUT_BEFORE_SEGMENTS = TAIL_Q_OUTFLOW(JB_IN)
    CALL INITIALIZE_TAIL_SEGMENT_STATE(JB_IN)
    CALL ADVANCE_TAIL_SEGMENT_STATES(JB_IN, JW_IN)
    PROFILE_VOL_CHECK = TAIL_PROFILE_VOLUME(JB_IN, JW_IN, TAIL_WSE_UP(JB_IN), TAIL_WSE_DN(JB_IN))
    PROFILE_VOL_GAP = TAIL_STORAGE_VOL(JB_IN)-PROFILE_VOL_CHECK
    IF (NIT <= 10 .OR. MOD(NIT,500) == 0 .OR. ABS(PROFILE_VOL_GAP) > 1.0D-3) THEN
      WARNING_OPEN = .TRUE.
      WRITE (WRN,'(A,1X,A,I0,5(1X,A,ES12.4))') '[V27_PROFILE_STORAGE]', 'JB=',JB_IN, &
        'VSTATE=',TAIL_STORAGE_VOL(JB_IN), 'VPROFILE=',PROFILE_VOL_CHECK, 'VGAP=',PROFILE_VOL_GAP, &
        'WUP=',TAIL_WSE_UP(JB_IN), 'WDN=',TAIL_WSE_DN(JB_IN)
    END IF
    TAIL_STORAGE_CHANGE_RATE(JB_IN) = (TAIL_STORAGE_VOL(JB_IN)-STORAGE_OLD)/MAX(ABS(DLT), 1.0D-12)
    TAIL_MASS_RESIDUAL(JB_IN) = TAIL_CONTINUITY_RESIDUAL( &
      TAIL_STORAGE_CHANGE_RATE(JB_IN)-(QPHYS-TAIL_Q_LINK(JB_IN)), &
      STORAGE_OLD, TAIL_STORAGE_VOL(JB_IN), DLT)
    TAIL_SEGMENT_Q_LOSS(JB_IN) = QOUT_BEFORE_SEGMENTS-TAIL_Q_LINK(JB_IN)
  ...
  REAL(R8) FUNCTION TAIL_CONTINUITY_RESIDUAL(RAW_RESIDUAL, VOLUME_OLD, VOLUME_NEW, DT_STEP)
    REAL(R8), INTENT(IN) :: RAW_RESIDUAL, VOLUME_OLD, VOLUME_NEW, DT_STEP
    REAL(R8) :: ROUND_BOUND

    ! A one-step subtraction of O(1e8 m3) R8 storage values has a finite
    ! representability floor.  Remove only the proven half-ULP projection
    ! bound; the established physical residual tolerance remains unchanged.
    ROUND_BOUND = 0.5D0*MAX(SPACING(VOLUME_OLD),SPACING(VOLUME_NEW))/ &
      MAX(ABS(DT_STEP),1.0D-12)
    TAIL_CONTINUITY_RESIDUAL = SIGN(MAX(ABS(RAW_RESIDUAL)-ROUND_BOUND,0.0D0),RAW_RESIDUAL)
  END FUNCTION TAIL_CONTINUITY_RESIDUAL
  ...
  REAL(R8) FUNCTION COMPUTE_TAIL_REACH_LENGTH(JB_IN, ISEG_FALLBACK)
    ...
    IF (TAIL_DOMAIN_DEFINED(JB_IN) .AND. TAIL_DNSEG(JB_IN) > TAIL_DOMAIN_US(JB_IN)) THEN
      DO IT=TAIL_DOMAIN_US(JB_IN),TAIL_DNSEG(JB_IN)-1
        COMPUTE_TAIL_REACH_LENGTH = COMPUTE_TAIL_REACH_LENGTH + 0.5D0*(DLX(IT)+DLX(IT+1))
      END DO
    END IF
  ...
  SUBROUTINE TAIL_STANDARD_RESIDUAL(IUP_SEG, IDN_SEG, JB_IN, JW_IN, QABS, WSE_DN, WSE_UP, RESIDUAL, OK)
    ...
    DX_REACH = MAX(TAIL_REACH_LENGTH(JB_IN), 1.0D0)
  ...
  SUBROUTINE COMPUTE_TAIL_LOCAL_LINK_FLOW(ISEG, JW_IN, WSE_UP, WSE_DN, DX_LOCAL, QOUT)
    INTEGER, INTENT(IN) :: ISEG, JW_IN
    REAL(R8), INTENT(IN) :: WSE_UP, WSE_DN, DX_LOCAL
    REAL(R8), INTENT(OUT) :: QOUT
    REAL(R8) :: AREA_UP, HRAD_UP, FMANN_UP, SLOCAL
    LOGICAL :: OK

    ! Keep this formula identical to the current upstream-section link closure so
    ! the diagnostic tests whether that closure can be promoted, not a new model.
    CALL TAIL_SECTION_PROPS(ISEG, JW_IN, WSE_UP, AREA_UP, HRAD_UP, OK)
    IF (.NOT. OK) THEN
      QOUT = 0.0D0
      RETURN
    END IF
    SLOCAL = (WSE_UP-WSE_DN)/MAX(DX_LOCAL,1.0D0)
    IF (SLOCAL <= 1.0D-8) THEN
      QOUT = 0.0D0
      RETURN
    END IF
    FMANN_UP = TAIL_FMANN(ISEG, JW_IN, HRAD_UP)
    QOUT = AREA_UP*HRAD_UP**0.6666666666666667D0*SQRT(SLOCAL)/MAX(FMANN_UP,1.0D-8)
  END SUBROUTINE COMPUTE_TAIL_LOCAL_LINK_FLOW

  SUBROUTINE COMPUTE_TAIL_MACRO_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, WSE_UP, WSE_DN, &
    QOUT, RCOEFF, RFRIC, RVEL, OK)
    INTEGER, INTENT(IN) :: JB_IN, JW_IN, IUP_SEG, IDN_SEG
    REAL(R8), INTENT(IN) :: WSE_UP, WSE_DN
    REAL(R8), INTENT(OUT) :: QOUT, RCOEFF, RFRIC, RVEL
    LOGICAL, INTENT(OUT) :: OK

    CALL COMPUTE_TAIL_AGGREGATE_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, WSE_UP, WSE_DN, &
      .FALSE., 0.0D0, QOUT, RCOEFF, RFRIC, RVEL, OK)
  END SUBROUTINE COMPUTE_TAIL_MACRO_LINK_FLOW

  SUBROUTINE COMPUTE_TAIL_PROFILE_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, WSE_UP, WSE_DN, &
    EFFECTIVE_MANNING, QOUT, RCOEFF, RFRIC, RVEL, OK)
    INTEGER, INTENT(IN) :: JB_IN, JW_IN, IUP_SEG, IDN_SEG
    REAL(R8), INTENT(IN) :: WSE_UP, WSE_DN, EFFECTIVE_MANNING
    REAL(R8), INTENT(OUT) :: QOUT, RCOEFF, RFRIC, RVEL
    LOGICAL, INTENT(OUT) :: OK

    CALL COMPUTE_TAIL_AGGREGATE_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, WSE_UP, WSE_DN, &
      .TRUE., EFFECTIVE_MANNING, QOUT, RCOEFF, RFRIC, RVEL, OK)
  END SUBROUTINE COMPUTE_TAIL_PROFILE_LINK_FLOW

  SUBROUTINE SOLVE_TAIL_PROFILE_STAGE(JB_IN, JW_IN, IUP_SEG, IDN_SEG, Q_TARGET, WSE_DN, &
    EFFECTIVE_MANNING, WSE_SOLVED, SOLVED)
    INTEGER, INTENT(IN) :: JB_IN, JW_IN, IUP_SEG, IDN_SEG
    REAL(R8), INTENT(IN) :: Q_TARGET, WSE_DN, EFFECTIVE_MANNING
    REAL(R8), INTENT(OUT) :: WSE_SOLVED
    LOGICAL, INTENT(OUT) :: SOLVED
    INTEGER :: J
    REAL(R8) :: WLOW, WHIGH, WMID, QLOW, QHIGH, QMID, RCOEFF, RFRIC, RVEL
    LOGICAL :: OK_LOW, OK_HIGH, OK_MID

    WLOW = TAIL_MIN_UPSTREAM_STAGE(JB_IN, WSE_DN)
    CALL COMPUTE_TAIL_PROFILE_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, WLOW, WSE_DN, &
      EFFECTIVE_MANNING, QLOW, RCOEFF, RFRIC, RVEL, OK_LOW)
    IF (.NOT. OK_LOW) QLOW = 0.0D0
    WHIGH = WLOW + 1.0D0
    DO J=1,40
      CALL COMPUTE_TAIL_PROFILE_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, WHIGH, WSE_DN, &
        EFFECTIVE_MANNING, QHIGH, RCOEFF, RFRIC, RVEL, OK_HIGH)
      IF (OK_HIGH .AND. QHIGH >= Q_TARGET) EXIT
      WHIGH = WHIGH + MAX(1.0D0,0.5D0*(WHIGH-WLOW))
    END DO
    IF (.NOT. OK_HIGH .OR. QHIGH < Q_TARGET) THEN
      WSE_SOLVED = WLOW
      SOLVED = .FALSE.
      RETURN
    END IF
    ...
    DO J=1,60
      ...
      CALL COMPUTE_TAIL_PROFILE_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, WMID, WSE_DN, &
        EFFECTIVE_MANNING, QMID, RCOEFF, RFRIC, RVEL, OK_MID)
      IF (.NOT. OK_MID) THEN
        WSE_SOLVED = WLOW
        SOLVED = .FALSE.
        RETURN
      END IF
      IF (ABS(QMID-Q_TARGET) < 1.0D-3 .OR. ABS(WHIGH-WLOW) < 1.0D-8) EXIT
      IF (QMID >= Q_TARGET) THEN
        WHIGH = WMID
      ELSE
        WLOW = WMID
      END IF
    ...
    WSE_SOLVED = WMID
    SOLVED = .TRUE.
  END SUBROUTINE SOLVE_TAIL_PROFILE_STAGE

  SUBROUTINE COMPUTE_TAIL_AGGREGATE_LINK_FLOW(JB_IN, JW_IN, IUP_SEG, IDN_SEG, WSE_UP, WSE_DN, &
    USE_PROFILE, FMANN_OVERRIDE, QOUT, RCOEFF, RFRIC, RVEL, OK)
    INTEGER, INTENT(IN) :: JB_IN, JW_IN, IUP_SEG, IDN_SEG
    REAL(R8), INTENT(IN) :: WSE_UP, WSE_DN, FMANN_OVERRIDE
    LOGICAL, INTENT(IN) :: USE_PROFILE
    REAL(R8), INTENT(OUT) :: QOUT, RCOEFF, RFRIC, RVEL
    LOGICAL, INTENT(OUT) :: OK
    INTEGER :: ISEG, ISEG_PREV, IS_LOCAL
    REAL(R8) :: WSE_LOCAL, AREA_LOCAL, HRAD_LOCAL, FMANN_LOCAL, RESIST_LOCAL, PROFILE_FRAC
    REAL(R8) :: AREA_PREV, RESIST_PREV, AREA_UP, AREA_DN, DX_LOCAL, HEAD_LOCAL
    LOGICAL :: SECTION_OK
    ...
    AREA_UP = 0.0D0
    AREA_DN = 0.0D0
    DO ISEG=IUP_SEG,IDN_SEG
      IF (USE_PROFILE) THEN
        PROFILE_FRAC = TAIL_PROFILE_FRACTION(JB_IN, ISEG)
        WSE_LOCAL = WSE_UP + PROFILE_FRAC*(WSE_DN-WSE_UP)
      ELSEIF (ISEG == IUP_SEG) THEN
        WSE_LOCAL = WSE_UP
      ELSEIF (ISEG == IDN_SEG) THEN
        WSE_LOCAL = WSE_DN
      ELSEIF (ISEG >= TAIL_DOMAIN_US(JB_IN) .AND. ISEG <= TAIL_DOMAIN_DS(JB_IN)) THEN
        IS_LOCAL = ISEG-TAIL_DOMAIN_US(JB_IN)+1
        WSE_LOCAL = TAIL_STAGE_SEG(IS_LOCAL,JB_IN)
      ELSE
        WSE_LOCAL = ELWS(ISEG)
      END IF
      CALL TAIL_SECTION_PROPS(ISEG, JW_IN, WSE_LOCAL, AREA_LOCAL, HRAD_LOCAL, SECTION_OK)
      IF (.NOT. SECTION_OK) RETURN
      IF (FMANN_OVERRIDE > 0.0D0) THEN
        FMANN_LOCAL = FMANN_OVERRIDE
      ELSE
        FMANN_LOCAL = TAIL_FMANN(ISEG, JW_IN, HRAD_LOCAL)
      END IF
      RESIST_LOCAL = (FMANN_LOCAL/MAX(AREA_LOCAL*HRAD_LOCAL**0.6666666666666667D0,1.0D-6))**2
      IF (ISEG == IUP_SEG) THEN
        AREA_UP = AREA_LOCAL
      ELSE
        DX_LOCAL = MAX(0.5D0*(DLX(ISEG_PREV)+DLX(ISEG)),1.0D0)
        RFRIC = RFRIC + 0.5D0*(RESIST_PREV+RESIST_LOCAL)*DX_LOCAL
      END IF
      ISEG_PREV = ISEG
      AREA_PREV = AREA_LOCAL
      RESIST_PREV = RESIST_LOCAL
    END DO
    AREA_DN = AREA_PREV
    RVEL = (1.0D0/MAX(AREA_DN,1.0D-6)**2 - &
      1.0D0/MAX(AREA_UP,1.0D-6)**2)/(2.0D0*G)
    RCOEFF = RFRIC + RVEL
    HEAD_LOCAL = WSE_UP-WSE_DN
    IF (HEAD_LOCAL <= 1.0D-8 .OR. RCOEFF <= 1.0D-16) RETURN
    QOUT = SQRT(HEAD_LOCAL/RCOEFF)
    OK = .TRUE.
  END SUBROUTINE COMPUTE_TAIL_AGGREGATE_LINK_FLOW
  ...
  REAL(R8) FUNCTION TAIL_PROFILE_FRACTION(JB_IN, ISEG)
    INTEGER, INTENT(IN) :: JB_IN, ISEG
    INTEGER :: IT
    REAL(R8) :: XDIST, XTOTAL

    XDIST = 0.0D0
    XTOTAL = COMPUTE_TAIL_REACH_LENGTH(JB_IN, TAIL_DOMAIN_US(JB_IN))
    IF (ISEG > TAIL_DOMAIN_US(JB_IN)) THEN
      DO IT=TAIL_DOMAIN_US(JB_IN),ISEG-1
        XDIST = XDIST + 0.5D0*(DLX(IT)+DLX(IT+1))
      END DO
    END IF
    TAIL_PROFILE_FRACTION = MIN(MAX(XDIST/MAX(XTOTAL,1.0D0),0.0D0),1.0D0)
  END FUNCTION TAIL_PROFILE_FRACTION

  REAL(R8) FUNCTION TAIL_PROFILE_VOLUME(JB_IN, JW_IN, WSE_UP, WSE_DN)
    INTEGER, INTENT(IN) :: JB_IN, JW_IN
    REAL(R8), INTENT(IN) :: WSE_UP, WSE_DN
    INTEGER :: ISEG
    REAL(R8) :: FRAC, WSE_SEG

    TAIL_PROFILE_VOLUME = 0.0D0
    IF (.NOT. TAIL_DOMAIN_DEFINED(JB_IN)) RETURN
    DO ISEG=TAIL_DOMAIN_US(JB_IN),TAIL_DOMAIN_DS(JB_IN)
      FRAC = TAIL_PROFILE_FRACTION(JB_IN, ISEG)
      WSE_SEG = WSE_UP + FRAC*(WSE_DN-WSE_UP)
      TAIL_PROFILE_VOLUME = TAIL_PROFILE_VOLUME + TAIL_VOLUME_FROM_STAGE(ISEG, JW_IN, WSE_SEG)
    END DO
  END FUNCTION TAIL_PROFILE_VOLUME

  REAL(R8) FUNCTION TAIL_MIN_UPSTREAM_STAGE(JB_IN, WSE_DN)
    INTEGER, INTENT(IN) :: JB_IN
    REAL(R8), INTENT(IN) :: WSE_DN
    INTEGER :: ISEG
    REAL(R8) :: FRAC, BED_MIN, REQUIRED_UP

    TAIL_MIN_UPSTREAM_STAGE = WSE_DN
    DO ISEG=TAIL_DOMAIN_US(JB_IN),TAIL_DOMAIN_DS(JB_IN)
      FRAC = TAIL_PROFILE_FRACTION(JB_IN, ISEG)
      BED_MIN = EL(KB(ISEG)+1,ISEG)+FRONT_H_MIN
      IF (1.0D0-FRAC > 1.0D-10) THEN
        REQUIRED_UP = (BED_MIN-FRAC*WSE_DN)/(1.0D0-FRAC)
        TAIL_MIN_UPSTREAM_STAGE = MAX(TAIL_MIN_UPSTREAM_STAGE, REQUIRED_UP)
      END IF
    END DO
  END FUNCTION TAIL_MIN_UPSTREAM_STAGE

  REAL(R8) FUNCTION TAIL_MIN_PROFILE_VOLUME(JB_IN, JW_IN, WSE_DN)
    INTEGER, INTENT(IN) :: JB_IN, JW_IN
    REAL(R8), INTENT(IN) :: WSE_DN
    REAL(R8) :: WSE_MIN_UP

    WSE_MIN_UP = TAIL_MIN_UPSTREAM_STAGE(JB_IN, WSE_DN)
    TAIL_MIN_PROFILE_VOLUME = TAIL_PROFILE_VOLUME(JB_IN, JW_IN, WSE_MIN_UP, WSE_DN)
  END FUNCTION TAIL_MIN_PROFILE_VOLUME

  SUBROUTINE INVERT_TAIL_PROFILE_STORAGE(JB_IN, JW_IN, TARGET_VOL, WSE_DN, WSE_OUT, STRICT)
    INTEGER, INTENT(IN) :: JB_IN, JW_IN
    REAL(R8), INTENT(IN) :: TARGET_VOL, WSE_DN
    REAL(R8), INTENT(OUT) :: WSE_OUT
    LOGICAL, INTENT(IN), OPTIONAL :: STRICT
    REAL(R8) :: WLOW, WHIGH, WMID, VLOW, VHIGH, VMID, VOL_TOL, STAGE_TOL
    INTEGER :: J
    ...
    IF (PRESENT(STRICT)) THEN
      ...
      END IF
    END IF
    VLOW = TAIL_PROFILE_VOLUME(JB_IN, JW_IN, WLOW, WSE_DN)
    ...
    WHIGH = WLOW + 1.0D0
    VHIGH = TAIL_PROFILE_VOLUME(JB_IN, JW_IN, WHIGH, WSE_DN)
    DO J=1,40
    ...
    DO J=1,60
      WMID = 0.5D0*(WLOW+WHIGH)
      VMID = TAIL_PROFILE_VOLUME(JB_IN, JW_IN, WMID, WSE_DN)
      IF (ABS(VMID-TARGET_VOL) < VOL_TOL .OR. ABS(WHIGH-WLOW) < STAGE_TOL) EXIT
    ...
    END DO
    WSE_OUT = WMID
  END SUBROUTINE INVERT_TAIL_PROFILE_STORAGE

  REAL(R8) FUNCTION EFFECTIVE_UPSTREAM_INFLOW(JB_IN)
    INTEGER, INTENT(IN) :: JB_IN
    INTEGER :: IUP_ACTIVE, JW_ACTIVE
    REAL(R8) :: QPHYS, VOL_MIN_ACTIVE, Q_AVAILABLE_MAX_ACTIVE

    QPHYS = MAX(QIN(JB_IN), 0.0D0)
    EFFECTIVE_UPSTREAM_INFLOW = QPHYS
    IF (UPSTREAM_DOMAIN_LOCK(JB_IN) .AND. TAIL_STAGE_VALID(JB_IN)) THEN
      IUP_ACTIVE = TAIL_UPSEG(JB_IN)
      JW_ACTIVE = MAX(1, MIN(NWB, WBSEG(IUP_ACTIVE)))
      VOL_MIN_ACTIVE = TAIL_MIN_PROFILE_VOLUME(JB_IN, JW_ACTIVE, TAIL_WSE_DN(JB_IN))
      Q_AVAILABLE_MAX_ACTIVE = QPHYS + &
        MAX(TAIL_STORAGE_VOL(JB_IN)-VOL_MIN_ACTIVE, 0.0D0)/MAX(ABS(DLT), 1.0D-12)
      ! HYDROINOUT may refresh physical Q after the tail predictor. Re-cap before
      ! the reservoir matrix consumes the flux so commit and reservoir stay exact.
      EFFECTIVE_UPSTREAM_INFLOW = MIN(MAX(TAIL_Q_LINK(JB_IN), 0.0D0), Q_AVAILABLE_MAX_ACTIVE)
      IF (TAIL_Q_LINK(JB_IN) > EFFECTIVE_UPSTREAM_INFLOW+1.0D-9 .AND. &
          (NIT <= 10 .OR. MOD(NIT,500) == 0)) THEN
        WARNING_OPEN = .TRUE.
        WRITE (WRN,'(A,1X,A,I0,3(1X,A,ES12.4))') '[V26_Q_RECAP]', 'JB=',JB_IN, &
          'QPRED=',TAIL_Q_LINK(JB_IN), 'QMAX=',Q_AVAILABLE_MAX_ACTIVE, &
          'QRES=',EFFECTIVE_UPSTREAM_INFLOW
      END IF
    END IF
